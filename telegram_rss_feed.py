...
FEED_LIMIT = int(os.getenv("TG_FEED_LIMIT", "7"))
...
# Telegram Blenq is stricter than the public Blenq page.
BLENQ_MIN_ODDS = 1.70
BLENQ_MAX_ODDS = 2.65
BLENQ_MAX_GAP = 10.0
BLENQ_MIN_WR = 60.0
...
# Production model feeds should already be filtered by build_pages.py, but the
# Telegram sender keeps a second safety layer to avoid sending malformed rows.
GENERAL_MIN_ODDS = 1.40
...
def number(label, description, percent=False):
    ...
    match = re.search(
        re.escape(label) + r":\s*([+-]?[0-9]+(?:\.[0-9]+)?)" + suffix,
        text,
        re.IGNORECASE,
    )
...
def general_allowed(description):
    odd = parse_float(number("Odds", description, False))
    win_rate = parse_float(number("Win probability", description, True))
    if odd is None or win_rate is None:
        return False
    if odd < GENERAL_MIN_ODDS:
        return False
    return True
...
def build_message(feed):
    ...
    for item in parsed.entries:
        ...
        if feed["key"] == "blenq":
            if not blenq_allowed(description):
                continue
        else:
            if not general_allowed(description):
                continue
        ...
        # Keep the percentage value, but do not print the literal "WR" label.
