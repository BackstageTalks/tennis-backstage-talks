...
def load_local_results():
    ...
    for file_name in sorted(files):
        ...
        try:
            ...
            print("LOCAL RESULT LOAD ERROR:", path, str(e))

    return payloads


def fetch_remote_result(date_value):
    url = f"{BASE}results_{date_value}.json?v=period-summary"

    try:
        response = requests.get(url, headers=HEADERS, timeout=15)

        print("FETCH REMOTE RESULT:", url, response.status_code)
        ...
        if isinstance(data, dict) and data.get("type") == "TOP7_RESULTS":
            return data

        return None

    except Exception as e:
        print("FETCH REMOTE RESULT ERROR:", url, str(e))
        return None


def load_results_history():
    """
    public sa v workflow čistí, preto históriu berieme:
    1. z lokálne vytvoreného results_YYYY-MM-DD.json
    2. z už publikovaných GitHub Pages results_YYYY-MM-DD.json
    """
    history = {}
...
<description>Automated TOP7 pick result review with units, weekly and monthly summaries</description>
{items}
</channel>
</rss>
"""
