...
def result_badge(text, status="neutral"):
    return f'<span class="result-badge result-{safe(status)}">{safe(text)}</span>'
...
def evaluate_score_prediction(item):
    ...
    if predicted_score:
        ...
        status = "hit" if score_hit else "miss"
        rows.append(
            " ".join([
                result_badge(f"Pred sets {predicted_text}", status),
                result_badge(f"Real sets {actual_set_score}", status),
                result_badge("Score HIT" if score_hit else "Score MISS", status),
            ])
        )
    else:
        rows.append(result_badge(f"Real sets {actual_set_score}", "neutral"))

    if total_games is not None:
        rows.append(result_badge(f"Real games {total_games}", "neutral"))
    ...
    if sets_label and actual_sets is not None:
        ...
        if expected_sets is not None:
            ...
            rows.append(
                " ".join([
                    result_badge(f"Pred {expected_sets} sets", "hit" if sets_hit else "miss"),
                    result_badge(f"Real {actual_sets} sets", "hit" if sets_hit else "miss"),
                    result_badge("Sets HIT" if sets_hit else "Sets MISS", "hit" if sets_hit else "miss"),
                ])
            )
    ...
    if games_pick and games_line not in [None, ""] and total_games is not None:
        try:
            ...
            if games_hit is not None:
                status = "hit" if games_hit else "miss"
                rows.append(
                    " ".join([
                        result_badge(f"Pred {games_pick}", status),
                        result_badge(f"Real {total_games}g", status),
                        result_badge("Games HIT" if games_hit else "Games MISS", status),
                    ])
                )
        except Exception:
            pass
...
def format_sets_games(item):
    ...
    if expected_sets not in [None, ""]:
        try:
            sets_bits.append(f"Sets {float(expected_sets):.2f}")
        except Exception:
            sets_bits.append(f"Sets {expected_sets}")

    sets_label = item.get("sets_probability_label")
    sets_probability = compact_pct(item.get("sets_probability"))
    if sets_label and sets_probability:
        sets_bits.append(f"{sets_label} {sets_probability}")

    most_likely_score = item.get("most_likely_score")
    if most_likely_score:
        sets_bits.append(f"Score {most_likely_score}")

    expected_games = item.get("expected_games")
    if expected_games not in [None, ""]:
        try:
            games_bits.append(f"Games {float(expected_games):.1f}")
        except Exception:
            games_bits.append(f"Games {expected_games}")

    games_pick = item.get("games_pick")
    games_line = item.get("games_line")
    games_over_probability = compact_pct(item.get("games_over_probability"))
    if games_pick:
        games_text = str(games_pick)
        if games_line not in [None, ""] and str(games_line) not in games_text:
            games_text = f"{games_text} {games_line}"
        if games_over_probability:
    ...
    lines = []
    if sets_bits:
        lines.append(safe(" • ".join(str(part) for part in sets_bits)))
    if games_bits:
        lines.append(safe(" • ".join(str(part) for part in games_bits)))
    if eval_text:
...
<tr>
    ...
    <td class="sets-games-cell">{format_sets_games(item)}</td>
