...
"""
    ...
    if not move_range:
        ...
        if initial_pick is not None and current_pick is not None and initial_pick > 1 and current_pick > 1:
            # Only manufacture Range/Move % from identical odds if there is a real
            # movement value already present. This avoids fake 1.57 -> 1.57 ranges.
    ...
    # Show Move % and Range only when the movement endpoint returned real
    # pre-snapshot movement data. This prevents fake rows like 1.57 -> 1.57.
    movement_available = prediction.get("marq_movement_available") is True
    show_move_detail = movement_available and move_pct is not None

    if show_move_detail:
        rows += metric_row("Move %", market_pct(move_pct))
        if move_range:
