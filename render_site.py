...
def render_thinq_box(prediction):
    ...
    elo_edge = format_thinq_edge(prediction.get("thinq_elo_edge"))
    h2h_edge = format_thinq_edge(prediction.get("thinq_h2h_edge"))
    ...
    rows = (
        ...
        + metric_row("ELO", elo_edge)
        + metric_row("H2H", h2h_edge)
        + metric_row("Games", projected_games)
        + metric_row("TB", tiebreak_probability)
        + metric_row("Decider", decider_probability)
    )
...
def render_rss(predictions, title, link):
    ...
    for prediction in predictions:
        ...
        description_text = (
            ...
            f"THINQ ELO Edge: {format_thinq_edge(prediction.get('thinq_elo_edge'))}\n"
            f"THINQ H2H Edge: {format_thinq_edge(prediction.get('thinq_h2h_edge'))}\n"
            f"THINQ Games: {format_thinq_games(prediction.get('thinq_projected_games') or prediction.get('expected_games'))}\n"
            f"THINQ T-Break: {format_thinq_probability(prediction.get('thinq_tiebreak_probability') or prediction.get('tie_break_probability'))}\n"
            f"THINQ Decider: {format_thinq_probability(prediction.get('thinq_decider_probability'))}\n"
