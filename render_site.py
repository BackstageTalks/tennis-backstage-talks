...
def resolve_score_display(prediction):
    score = prediction.get("most_likely_score")
    if not score:
        return ""
    player1 = prediction.get("player1") or "Player 1"
    player2 = prediction.get("player2") or "Player 2"
    return f"{player1} {score} {player2}"
...
"""
    ...
    pick_market_display = market_pct(crowd_pick)
    opponent_market_display = market_pct(crowd_opponent)
    market_lean_display = resolve_market_lean(prediction)
    ...
    rows = (
        metric_row("Pick Mkt", pick_market_display)
        + metric_row("Opp Mkt", opponent_market_display)
        + metric_row("Lean", market_lean_display)
...
def render_sets_box(expected_sets, sets_probability_label, sets_probability, most_likely_html, prediction=None):
    ...
    tie_break_probability = prediction.get("tie_break_probability")
    ...
    if tie_break_probability is not None:
        extra_rows += metric_row("T-Break", yes_no_market_probability(tie_break_probability))
...
def render_rows(predictions, model_view="corq"):
    ...
    for index, prediction in enumerate(predictions, start=1):
        ...
        most_likely_score = safe(resolve_score_display(prediction), default="")
        ...
        most_likely_html = metric_row("P1/P2", most_likely_score) if most_likely_score else ""
...
def render_rss(predictions, title, link):
    ...
    for prediction in predictions:
        ...
        description_text = (
            ...
            f"MARQ Pick Market: {marq_pick_market}\nMARQ Opp Market: {marq_opp_market}\nMARQ Market Lean: {marq_market_lean}\n"
