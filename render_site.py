...
def render_data_ai_box(prediction):
    ...
    corq_probability = prediction.get("corq_ai_probability") or prediction.get("probability")
    ...
    if status != "OK":
        ...
        <div class="intel-box data-ai-box">
            ...
            <div class="intel-row"><span>Corq AI</span><span>{pct(corq_probability)}</span></div>
...
"""
    ...
    return f"""
        <div class="intel-box data-ai-box">
            <div class="box-title">DATA AI</div>
            <div class="intel-row ai-main-row"><span>AI Match</span><span>{pct_plain(ai_match)}</span></div>
            <div class="intel-row"><span>Corq AI</span><span>{pct(corq_probability)}</span></div>
            <div class="intel-row"><span>BsT AI</span><span>{pct(bst_probability)}</span></div>
        </div>
"""
...
def render_page(predictions, title, subtitle):
    ...
    page_subtitle = safe(subtitle or HEADER_SUBTITLE)
...
<style>
:root {{ --bg:#0f172a; --panel:#111827; --panel-2:#1e293b; --border:#334155; --text:#e5e7eb; --muted:#94a3b8; --green:#22c55e; --yellow:#facc15; --blue:#38bdf8; }}
...
.intel {{ line-height:1.45; min-width:430px; }}
...
.intel-layout {{ display:grid; grid-template-columns:128px 128px 128px; gap:10px; align-items:stretch; }}
.intel-box {{ min-height:104px; padding:9px 10px; border-radius:8px; font-size:11px; line-height:1.45; background:rgba(100,116,139,.12); border:1px solid rgba(148,163,184,.55); }}
.box-title {{ color:#e5e7eb; font-size:11px; font-weight:900; text-transform:uppercase; letter-spacing:.05em; margin-bottom:7px; }}
.intel-row {{ display:grid; grid-template-columns:64px auto; column-gap:7px; align-items:center; margin-top:4px; }}
.intel-row span:last-child {{ text-align:right; color:var(--text); font-weight:800; }}
.ai-main-row {{ color:var(--blue); font-weight:900; }}
...
@media (max-width:1200px) {{ .intel-layout {{ grid-template-columns:1fr; }} .intel {{ min-width:180px; }} }}
