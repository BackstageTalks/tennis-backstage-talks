...
def resolve_ai_metrics(prediction):
    ...
    blenq_pct = normalize_probability_for_display(prediction.get("blenq_probability") or prediction.get("blend_probability"))
    return {"corq_pct": corq_pct, "thinq_pct": thinq_pct, "blenq_pct": blenq_pct, "ai_match": ai_match}
...
def resolve_model_view(title, subtitle):
    ...
    if "blenq" in text or "blend" in text:
        return "blenq"
...
def render_data_ai_box(prediction, model_view="corq"):
    ...
    blenq_display = format_pct_points(metrics.get("blenq_pct"))
    ...
    if model_view == "blenq":
        rows = (
            metric_row("Blenq", blenq_display)
            + metric_row("Edge", format_edge(prediction.get("edge_pp")))
            + metric_row("Gap", pct(prediction.get("market_gap")))
            + metric_row("Market", pct(prediction.get("market_fair_pick")))
            + metric_row("Corq", corq_display)
            + metric_row("Thinq", thinq_display)
            + metric_row("AI Match", ai_match_display)
            + metric_row("Tier", prediction.get("blenq_tier"))
        )
    elif model_view == "thinq":
...
def render_marq_ai_box(prediction):
    ...
    return f"""
        <div class="intel-panel marq-panel">
            ...
            {score_html}
            {metric_row("Risk", prediction.get("blenq_risk")) if prediction.get("blenq_risk") else ""}
...
def render_rows(predictions, model_view="corq"):
    ...
    for index, prediction in enumerate(predictions, start=1):
        ...
        blenq_meta_html = ""
        if model_view == "blenq":
            blenq_meta_html = f'<div class="match-meta blenq-meta">Gap {pct(prediction.get("market_gap"))} • Edge {format_edge(prediction.get("edge_pp"))} • {safe(prediction.get("blenq_tier"))}</div>'
...
.market-thin {{ color:#a78bfa; border:1px solid rgba(167,139,250,.45); background:rgba(167,139,250,.08); }}
...
def render_rss(predictions, title, link):
    ...
    for prediction in predictions:
        ...
        description_text = (
            ...
            f"Corq AI: {corq_ai}\nThinq AI: {thinq_ai}\nBlenq: {format_pct_points(metrics.get("blenq_pct"))}\nAI Match: {ai_match}\nAI Difference: {delta['label']}\n"
            f"Market fair pick: {pct(prediction.get("market_fair_pick"))}\nMarket gap: {pct(prediction.get("market_gap"))}\nEdge: {format_edge(prediction.get("edge_pp"))}\nBlenq tier: {safe(prediction.get("blenq_tier"))}\nBlenq risk: {safe(prediction.get("blenq_risk"))}\nMarq AI: {marq_score}\nMarket Signal: {marq_signal}\nExpected sets: {expected_sets}\n"
