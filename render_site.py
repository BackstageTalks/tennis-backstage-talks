...
"""
    ...
    pick_marq_display = market_pct(crowd_pick)
    opponent_marq_display = market_pct(crowd_opponent)
    ...
    tie_break_display = yes_no_market_probability(prediction.get("tie_break_probability"))
    h2h_display = resolve_h2h_display(prediction)

    rows = (
        metric_row("Pick Marq", pick_marq_display)
        + metric_row("Opp Marq", opponent_marq_display)
        + metric_row("Lean", market_lean_display)
        + metric_row("Move", move_display)
    )
    if tie_break_display != "-":
        rows += metric_row("T-Break", tie_break_display)
    if h2h_display != "-":
        rows += metric_row("H2H", h2h_display)
    rows += (
...
def render_rss(predictions, title, link):
    ...
    for prediction in predictions:
        ...
        description_text = (
            ...
            f"MARQ Move: {marq_move}\nMARQ T-Break: {marq_tie_break}\nMARQ H2H: {marq_h2h}\n"
            f"MARQ Sharp: {marq_sharp}\nMARQ Quality: {marq_quality}\nMARQ CLV: {marq_clv}\n"
