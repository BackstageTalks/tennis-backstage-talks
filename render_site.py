...
def first_float_from_keys(item, keys):
    for key in keys:
        try:
            value = item.get(key)
...
def same_player_name(a, b):
    a_norm = normalize_player_name(a)
    b_norm = normalize_player_name(b)
...
def pick_odds_value(prediction):
    return first_float_from_keys(
        prediction,
        ["pick_odds", "odds", "marq_current_pick_odds", "marq_initial_pick_odds"],
    )


def opponent_odds_value(prediction):
    direct = first_float_from_keys(
        prediction,
        ["opponent_odds", "marq_current_opponent_odds", "marq_initial_opponent_odds"],
    )
    if direct is not None:
        return direct

    odds1 = first_float_from_keys(prediction, ["odds_player1", "p1_odds", "home_odds", "odds1", "price1"])
    odds2 = first_float_from_keys(prediction, ["odds_player2", "p2_odds", "away_odds", "odds2", "price2"])
    pick = prediction.get("pick")
    opponent = prediction.get("opponent")
    player1 = prediction.get("player1")
    player2 = prediction.get("player2")

    if same_player_name(opponent, player1) and odds1 is not None:
        return odds1
    if same_player_name(opponent, player2) and odds2 is not None:
        return odds2
    if same_player_name(pick, player1) and odds2 is not None:
        return odds2
    if same_player_name(pick, player2) and odds1 is not None:
        return odds1
    return None


def odds_badge(label, value, badge_class):
    try:
        number = float(value)
    except Exception:
        return ""
    return f'<span class="odds-badge {safe(badge_class)}">{safe(label)} @{odds(number)}</span>'
...
def render_sets_box(prediction, expected_sets, sets_probability_label, sets_probability, most_likely_html):
    games_value = prediction.get("expected_games") or prediction.get("thinq_projected_games")
    games_display = format_thinq_games(games_value)
    games_pick = prediction.get("games_pick")
    games_line = prediction.get("games_line")
    if games_pick and games_line not in [None, ""]:
        games_display = f"{games_display} / {games_pick}"
    elif games_pick:
        games_display = str(games_pick)

    tiebreak_display = format_thinq_probability(
        prediction.get("tie_break_probability")
        or prediction.get("thinq_tiebreak_probability")
        or prediction.get("tiebreak_probability")
    )
    decider_display = format_thinq_probability(prediction.get("thinq_decider_probability"))

    return f"""
        <div class="intel-panel sets-panel">
            <div class="panel-title">SETS / GAMES</div>
            {metric_row("Sets", expected_sets)}
            {metric_row(sets_probability_label, sets_probability)}
            {metric_row("Games", games_display)}
            {metric_row("TB", tiebreak_display)}
            {metric_row("Decider", decider_display)}
            {most_likely_html}
        </div>
"""


def format_thinq_confidence(value):
    number = safe_float(value)
    if number is None:
        return "-"
    if number <= 1.0:
...
def format_thinq_probability(value):
    number = safe_float(value)
    if number is None:
        return "-"
    if number <= 1.0:
...
def edge_from_pick_view(prediction, raw_edge):
    """Convert a player1-vs-player2 THINQ edge into PICK-view edge.

    THINQ raw edges are oriented as player1 minus player2.
    For UI we want: positive = supports the displayed pick.
    """
    value = safe_float(raw_edge)
    if value is None:
        return None
    ...
    if not player1 or not player2:
        match_p1, match_p2 = players_from_match_text(prediction.get("match"))
        player1 = player1 or match_p1
        player2 = player2 or match_p2

    if same_player_name(pick, player1):
        return value
    if same_player_name(pick, player2):
        return -value
    return value


def format_thinq_pick_edge(prediction, raw_edge):
    return format_thinq_edge(edge_from_pick_view(prediction, raw_edge))


def has_no_h2h_data(prediction):
    status = str(prediction.get("thinq_h2h_status") or prediction.get("h2h_signal") or "").upper()
    if status in {"NO_DATA", "NO H2H DATA", "NONE"}:
        return True
    flags = prediction.get("thinq_flags")
    if isinstance(flags, list) and any(str(flag).upper() == "NO_H2H_DATA" for flag in flags):
        return True
    for key in ["h2h_total_matches", "h2h_raw_count", "thinq_h2h_total_matches"]:
...
def format_thinq_h2h_edge(prediction):
    if has_no_h2h_data(prediction):
        return "No data"
    return format_thinq_pick_edge(prediction, prediction.get("thinq_h2h_edge"))


def format_thinq_games(value):
    number = safe_float(value)
    if number is None:
        return "-"
    return f"{number:.1f}"
...
def resolve_thinq_status(prediction):
    if prediction.get("thinq_available") is True:
        return "OK"
    error = prediction.get("thinq_error")
    if error:
        return "Error"
    return "No data"
...
def render_thinq_box(prediction):
    status = resolve_thinq_status(prediction)
    confidence = format_thinq_confidence(prediction.get("thinq_confidence"))
    elo_edge = format_thinq_pick_edge(prediction, prediction.get("thinq_elo_edge"))
    h2h_edge = format_thinq_h2h_edge(prediction)
    projected_games = format_thinq_games(prediction.get("thinq_projected_games") or prediction.get("expected_games"))
    tiebreak_probability = format_thinq_probability(
        prediction.get("thinq_tiebreak_probability") or prediction.get("tie_break_probability")
    )
    decider_probability = format_thinq_probability(prediction.get("thinq_decider_probability"))
    adjustment = format_thinq_edge(prediction.get("corq_thinq_adjustment"))

    rows = (
        metric_row("Conf", confidence)
        + metric_row("ELO PICK", elo_edge)
        + metric_row("H2H", h2h_edge)
    )

    if status != "OK":
        badge_class = "thinq-unavailable"
        badge_text = status
    else:
        badge_class = "thinq-ok"
        badge_text = f"Adj {adjustment}"

    return f"""
        <div class="intel-panel thinq-panel">
            <div class="panel-title">THINQ</div>
            {rows}
            <div class="thinq-badge {safe(badge_class)}">{safe(badge_text)}</div>
        </div>
"""
...
def render_match_intelligence(
    ...
    return f"""
        ...
        <div class="intel-layout">
            ...
            {render_sets_box(prediction, expected_sets, sets_probability_label, sets_probability, most_likely_html)}
            {render_thinq_box(prediction)}
        </div>
"""
...
def render_rows(predictions, model_view="corq"):
    ...
    for index, prediction in enumerate(predictions, start=1):
        ...
        pick_odds = pick_odds_value(prediction)
        opponent_odds = opponent_odds_value(prediction)
        odd = odds(pick_odds)
        pick_odds_html = odds_badge("Pick", pick_odds, "pick-odds-badge")
        opponent_odds_html = odds_badge("Opp", opponent_odds, "opponent-odds-badge")
...
<tr>
    ...
    <td class="pick-cell">
        ...
        <div class="pick-odds-under">{pick_odds_html}</div>
    ...
    <td class="opponent-cell">
        ...
        <div class="opponent-odds-under">{opponent_odds_html}</div>
    </td>
    <td>{time}</td>
    <td class="probability">{probability}</td>
    <td class="odds"><span class="odds-main-pill">{odd}</span></td>
...
.pick-sub {{ color:var(--green); font-size:13px; line-height:1.1; margin-top:5px; font-weight:900; }}
...
.opponent-name {{ font-size:16px; line-height:1.15; font-weight:900; color:var(--text); }}
.pick-odds-under, .opponent-odds-under {{ margin-top:6px; line-height:1.1; }}
.pick-odds-under {{ min-height:22px; }}
.opponent-odds-under {{ min-height:22px; }}
.odds-badge {{ display:inline-block; padding:4px 8px; border-radius:999px; font-size:11px; font-weight:900; line-height:1.1; white-space:nowrap; }}
.pick-odds-badge {{ color:#22c55e; background:rgba(34,197,94,.16); border:1px solid rgba(34,197,94,.45); }}
.opponent-odds-badge {{ color:#94a3b8; background:rgba(100,116,139,.16); border:1px solid rgba(100,116,139,.38); }}
.probability {{ font-weight:800; color:var(--green); }}
.odds {{ font-weight:800; color:var(--yellow); white-space:nowrap; }}
.odds-main-pill {{ display:inline-block; padding:5px 9px; border-radius:999px; color:#facc15; background:rgba(250,204,21,.14); border:1px solid rgba(250,204,21,.45); }}
...
.intel-layout {{ display:grid; grid-template-columns:minmax(110px,1fr) minmax(110px,1fr) minmax(130px,1.15fr) minmax(130px,1.15fr); gap:8px; align-items:stretch; }}
...
.thinq-badge {{ display:inline-block; margin-top:8px; padding:5px 10px; border-radius:5px; font-size:10px; font-weight:900; letter-spacing:.04em; text-transform:uppercase; }}
.thinq-ok {{ color:#38bdf8; border:1px solid rgba(56,189,248,.45); background:rgba(56,189,248,.08); }}
.thinq-unavailable {{ color:#94a3b8; border:1px solid rgba(148,163,184,.35); background:rgba(148,163,184,.05); text-transform:none; }}
...
def render_rss(predictions, title, link):
    ...
    for prediction in predictions:
        ...
        marq_tie_break = yes_no_market_probability(
            prediction.get("tie_break_probability")
            or prediction.get("marq_tie_break")
            or prediction.get("tiebreak_probability")
            or prediction.get("tie_break")
        )
        marq_h2h = safe(resolve_h2h_display(prediction), default="-")
        ...
        description_text = (
            ...
            f"MARQ T-Break: {marq_tie_break}\nMARQ H2H: {marq_h2h}\n"
            f"MARQ Quality: {marq_quality}\nMARQ CLV: {marq_clv}\n"
            f"THINQ Available: {prediction.get('thinq_available')}\n"
            f"THINQ Confidence: {format_thinq_confidence(prediction.get('thinq_confidence'))}\n"
            f"THINQ ELO Pick Edge: {format_thinq_pick_edge(prediction, prediction.get('thinq_elo_edge'))}\n"
            f"THINQ H2H Edge: {format_thinq_h2h_edge(prediction)}\n"
            f"Expected sets: {expected_sets}\n{sets_label}: {sets_probability}\n"
            f"Expected games: {format_thinq_games(prediction.get('expected_games') or prediction.get('thinq_projected_games'))}\n"
            f"Games pick: {safe(prediction.get('games_pick'), default='-')}\n"
            f"Tie-break probability: {format_thinq_probability(prediction.get('tie_break_probability') or prediction.get('thinq_tiebreak_probability') or prediction.get('tiebreak_probability'))}\n"
            f"Decider probability: {format_thinq_probability(prediction.get('thinq_decider_probability'))}\n"
