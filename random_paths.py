...
def build_nav_html(active_key=""):
    links = [
        ...
        html_link(f"{BASE_URL}/{BLEND_PATH}/", "Blenq", active_key == "blend"),
        html_link(f"{BASE_URL}/{CORQ_RSS_PATH}", "Corq RSS"),
        html_link(f"{BASE_URL}/{THINQ_RSS_PATH}", "Thinq RSS"),
        html_link(f"{BASE_URL}/{BLEND_RSS_PATH}", "Blenq RSS"),
...
<rss version="2.0">
<channel>
<title>BackstageTalks Statistical Engine - Blenq</title>
<link>{BASE_URL}/{BLEND_PATH}/</link>
<description>Blenq RSS feed is available after Blenq predictions are generated.</description>
</channel>
</rss>
'''
...
def create_random_page_outputs():
    ...
    if f"public/{BLEND_PATH}/index.html" not in copied:
        create_placeholder_page(
            ...
            "Blenq",
            "Blenq page is prepared. Blenq predictions will appear here after build_pages.py generates public/Blend/index.html.",
        )
