...
def active_key_for_path(path):
    ...
    if normalized == f"public/{THINQ_PATH}/index.html":
        return "thinq"
    if normalized == f"public/{CLOQ_PATH}/index.html":
        return "cloq"
...
def build_nav_html(active_key=""):
    links = [
        ...
        html_link(f"{BASE_URL}/{CLOQ_PATH}/", "Cloq", active_key == "cloq"),
        html_link(f"{BASE_URL}/{CORQ_RSS_PATH}", "Corq RSS"),
        html_link(f"{BASE_URL}/{THINQ_RSS_PATH}", "Thinq RSS"),
        html_link(f"{BASE_URL}/{CLOQ_RSS_PATH}", "Cloq RSS"),
...
<rss version="2.0">
<channel>
<title>BackstageTalks Statistical Engine - Cloq</title>
<link>{BASE_URL}/{CLOQ_PATH}/</link>
<description>Cloq RSS feed is available after Cloq predictions are generated.</description>
</channel>
</rss>
'''
...
def create_random_page_outputs():
    ...
    page_map = [
        ...
        ("public/Cloq/index.html", f"public/{CLOQ_PATH}/index.html"),
    ...
    if f"public/{CLOQ_PATH}/index.html" not in copied:
        create_placeholder_page(
            f"public/{CLOQ_PATH}/index.html",
            "Cloq",
            "Cloq page is prepared. Cloq predictions will appear here after build_pages.py generates public/Cloq/index.html.",
        )
        copied.append(f"public/{CLOQ_PATH}/index.html")
...
def create_random_rss_outputs():
    rss_map = [
        ...
        ("public/tennis_cloq.xml", f"public/{CLOQ_RSS_PATH}", f"{BASE_URL}/{CLOQ_PATH}/"),
    ]
    ...
    if not Path(f"public/{CLOQ_RSS_PATH}").exists():
        create_placeholder_rss(f"public/{CLOQ_RSS_PATH}")
...
def remove_old_public_paths():
    old_paths = [
        ...
        "public/tennis_cloq.xml",
        "public/tennis_" + "bl" + "end.xml",
