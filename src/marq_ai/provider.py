...
def _name_score(a: str, b: str) -> float:
    ...
    if ta and tb:
        ta_last = sorted(ta)[-1]
        tb_last = sorted(tb)[-1]
        if ta_last in tb or tb_last in ta:
...
def fetch_provider_odds(event_id: str, provider_id: int, force_refresh: bool = False) -> Optional[Dict[str, Any]]:
    ...
    # getAllOddsForEvent path still needs final confirmation; first candidate is likely.
    # winning-odds path was verified earlier and is kept as fallback.
    candidates = [
