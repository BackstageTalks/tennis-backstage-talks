...
_RUN_EVENTS_CACHE: Dict[str, List[Dict[str, Any]]] = {}
_RUN_FAILED_DATES: set[str] = set()
_RUN_ODDS_CACHE: Dict[str, Optional[Dict[str, Any]]] = {}
_RATE_LIMITED = False
...
def _name_score(a: str, b: str) -> float:
    ...
    if ta and tb:
        ta_list = list(ta)
        tb_list = list(tb)
        if ta_list[-1] in tb or tb_list[-1] in ta:
...
def _get_json(path: str, cache_name: Optional[str] = None, force_refresh: bool = False) -> Optional[Any]:
    ...
    try:
        ...
        _debug(
            f"http status={status} path={path} content_type={content_type} body_preview={text_preview}"
        )

        if status == 204:
            _debug(f"no content path={path}")
            return None
        ...
        if status >= 400:
            _debug(f"http error status={status} path={path}")
            return None

        if not response.text or not response.text.strip():
            _debug(f"empty response path={path}")
            return None
...
def fetch_events_by_date(date_only: str, force_refresh: bool = False) -> List[Dict[str, Any]]:
    if date_only in _RUN_EVENTS_CACHE and not force_refresh:
        return _RUN_EVENTS_CACHE[date_only]
    if date_only in _RUN_FAILED_DATES and not force_refresh:
        _debug(f"events skipped after previous failure date={date_only}")
        return []
    ...
    if not events:
        ...
        _RUN_EVENTS_CACHE[date_only] = events

    sample = []
    for event in events[:3]:
        home, away = _event_home_away(event)
        sample.append({"id": _event_id(event), "home": home, "away": away})
    _debug(f"events date={date_only} count={len(events)} sample={sample}")
    return events
...
def fetch_event_odds(event_id: str, force_refresh: bool = False) -> Optional[Dict[str, Any]]:
    if event_id in _RUN_ODDS_CACHE and not force_refresh:
        return _RUN_ODDS_CACHE[event_id]
    path = f"/api/tennis/event/{event_id}/odds"
    cache_name = f"tennisapi_odds_{event_id}.json"
    payload = _get_json(path, cache_name=cache_name, force_refresh=force_refresh)
    _RUN_ODDS_CACHE[event_id] = payload
    return payload
...
def parse_event_odds(payload: Any) -> Optional[Dict[str, Any]]:
    ...
    if not market:
        _debug("odds parse failed: no market")
        return None
    choices = _extract_choices(market)
    if "1" not in choices or "2" not in choices:
        _debug(f"odds parse failed: choices missing market={market}")
        return None
    return {
...
def fetch_marq_market_data(
    ...
    **_: Any,
    ...
    if event_match:
        ...
        if parsed:
            ...
            result = {
                ...
                "odds": {
                    "od1": {"opening": c1.get("opening"), "latest": c1.get("current"), "current": c1.get("current"), "change": c1.get("change")},
                    "od2": {"opening": c2.get("opening"), "latest": c2.get("current"), "current": c2.get("current"), "change": c2.get("change")},
                },
                "raw": raw_odds,
            }
            _debug(
    ...
    if odds_player1 is not None and odds_player2 is not None:
        try:
            od1 = float(odds_player1)
            od2 = float(odds_player2)
            if od1 > 1 and od2 > 1:
                _debug(f"using fallback existing odds player1={player1} player2={player2} od1={od1} od2={od2}")
                return {
                    ...
                    "pick_outcome_key": _resolve_pick_outcome_key(player1, player2, pick),
                    "market_name": "current_odds_fallback",
            ...
            _debug(f"fallback existing odds failed error={exc}")
