...
def _build_bet365_marq_market_data(player1: str, player2: str, date_only: str, pick: Optional[str], force_refresh: bool = False) -> Optional[Dict[str, Any]]:
    ...
    pick_outcome_key = _resolve_pick_outcome_key(player1, player2, pick)
    snapshots = _save_bet365_snapshot(event_id, od1, od2)
    movement = _build_bet365_movement(snapshots, pick_outcome_key)

    opening1 = snapshots[0].get("odds1") if snapshots else od1
    opening2 = snapshots[0].get("odds2") if snapshots else od2
