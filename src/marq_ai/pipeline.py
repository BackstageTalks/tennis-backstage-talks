...
def _as_float(value: Any) -> Optionaltry:
...
def _safe_round(value: Any, digits: int = 2) -> Optionaltry:
        if value is None:
            return None
...
def _median_or_none(values: List[float]) -> Optionalclean = [float(v) for v in values if _as_float(v) is not None]
    if not clean:
        return None
...
def _candidate_keys(side: str, kind: str = "current") -> List"""
...
def _quote_side_odds(
    ...
    side: str,
    kind: str = "current",
) -> Optionalfor key in _candidate_keys(side, kind):
...
def _quote_player_odds(
    ...
    match_direction: str,
    kind: str = "current",
) -> Optionalside = _quote_player_side(player_outcome_key, match_direction)
...
def _quote_pick_odds(
    ...
    match_direction: str,
    kind: str = "current",
) -> Optionalreturn _quote_player_odds(
        quote=quote,
...
def _sharp_probability_from_quotes(
    ...
    outcome_key: str,
    match_direction: str,
) -> Optionalsharp_pick_probs: List[float] = []
