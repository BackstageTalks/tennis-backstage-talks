...
def _score_from_market_data(market_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # If opening equals latest, use neutral market score but still expose odds.
    if move_pct is None or prob_change_pp is None:
        score = 50
    else:
        raw_score = 50.0
        # Decimal odds down = implied probability up = market support.
        ...
        # Opponent confirmation: opponent odds drifting upward supports selected pick.
    ...
    consistency = 50
    if move_pct is not None and opponent_move_pct is not None:
...
def build_marq_from_match(
    ...
    **kwargs: Any,
    ...
    """
    Main Marq AI entrypoint used by prediction_engine_core.py.

    Primary datasource is TennisApi on RapidAPI:
    - /api/tennis/events/{day}/{month}/{year}
    - /api/tennis/event/{event_id}/odds

    Fallback can use already known odds_player1 / odds_player2 if supplied.
    """
