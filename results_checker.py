...
def coerce_void_after_evaluation(item):
    """
    Final safety net for historical or cached results:
    if a pick is settled as WON/LOST but has a declared winner and an incomplete
    tennis score such as "2-6 5-5", force it to VOID.
    """
    ...
    if winner and score and score_is_incomplete_text(score, best_of=best_of):
        updated = dict(item)
        updated["result_status"] = "VOID"
        updated["units"] = 0.0
        updated["result_void_reason"] = f"incomplete_score_with_winner:{score}"
        updated["resolved_at"] = now_utc_iso()
            ...
            _DEBUG.setdefault("examples_void", []).append(
                {
                    ...
                    "score": score,
                    "winner": winner,
                    "reason": updated["result_void_reason"],
                    "source": "post_evaluation_safety_net",
                }
            )
