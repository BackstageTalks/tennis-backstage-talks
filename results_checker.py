...
def score_is_incomplete_text(score, best_of=None):
    return completed_match_winner_from_score(score, best_of=best_of) is None if score else False


def should_recheck_previous_status(pick, previous):
    """
    Re-check previously settled picks when the old settlement can plausibly be wrong.

    Important case:
    A retirement/scratch may have been stored as LOST before VOID logic existed.
    Example score: "2-6 5-5" with a winner. That must be recalculated as VOID.
    """
    ...
    # Already VOID is safe to preserve.
    ...
    # If an old WON/LOST has a winner but incomplete score, it was likely a retirement.
    # Do not preserve it; re-run evaluation so is_void_match() can convert it to VOID.
    if previous_winner and previous_score and score_is_incomplete_text(previous_score, best_of=previous_best_of):
        return True

    try:
        pick_date = parse_date(pick.get("date"))
        today_date = parse_date(betting_day())
        if pick_date and today_date:
            # Re-check recent resolved items for a few days, because API statuses can mature
            # from partial/finished-like output into retired/walkover/void details.
            recheck_days = int(os.getenv("RESULTS_RECHECK_DAYS", "14"))
            if (today_date - pick_date).days <= recheck_days:
                return True
    except Exception:
        pass
...
def merge_previous_status(pick, previous_by_key):
    ...
    # Always re-evaluate today's betting day.
    ...
    # Re-evaluate old results that may have been incorrectly settled before VOID support.
    if should_recheck_previous_status(pick, previous):
        merged = dict(pick)
        for key in [
            "result_status",
            "winner",
