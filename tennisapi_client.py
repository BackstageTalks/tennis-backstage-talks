...
class TennisApiClient:
    ...
    def get_calendar_categories(
        ...
        """
        Correct TennisApi PRO daily category discovery endpoint.

        Provider-confirmed path:
            /api/tennis/calendar/{day}/{month}/{year}/categories

        This replaces the retired flat endpoint:
            /api/tennis/events/{day}/{month}/{year}
        """
        path = f"/api/tennis/calendar/{day}/{month}/{year}/categories"
    ...
    # Backward-compatible aliases used by cache/fetcher helpers.
    def get_categories_for_date(self, day: int, month: int, year: int) -> List[Dict[str, Any]]:
        return self.get_calendar_categories(day, month, year)

    def get_tennis_calendar_categories(self, day: int, month: int, year: int) -> List[Dict[str, Any]]:
        return self.get_calendar_categories(day, month, year)
    ...
    # Backward-compatible aliases used by cache helpers.
    def get_category_events(
        ...
        return self.get_events_by_category_date(category_id, day, month, year)
    ...
    def get_tennis_category_events(
        self,
        category_id: int,
        day: int,
        month: int,
    ...
    def get_events_by_date(
        ...
        discovered_categories = self.get_calendar_categories(day, month, year)
        discovered_ids: List[int] = []

        for category in discovered_categories:
            if not isinstance(category, dict):
                continue
            raw_id = category.get("id") or category.get("categoryId") or category.get("category_id")
            try:
                discovered_ids.append(int(raw_id))
            except Exception:
                pass

        scan_ids = discovered_ids or category_ids or []
        all_events: List[Dict[str, Any]] = []

        for category_id in scan_ids:
            try:
    ...
    # Common aliases for odds cache compatibility.
    def get_daily_odds(self, day: int, month: int, year: int) -> Dict[str, Any]:
        return self.get_events_odds_by_date(day, month, year)

    def get_events_odds(self, day: int, month: int, year: int) -> Dict[str, Any]:
        return self.get_events_odds_by_date(day, month, year)

    def get_tennis_events_odds(self, day: int, month: int, year: int) -> Dict[str, Any]:
        return self.get_events_odds_by_date(day, month, year)

    def get_tennis_daily_odds(self, day: int, month: int, year: int) -> Dict[str, Any]:
        return self.get_events_odds_by_date(day, month, year)

    def get_events_odds_for_date(self, day: int, month: int, year: int) -> Dict[str, Any]:
        return self.get_events_odds_by_date(day, month, year)
    ...
    def get_match_betting_odds(self, match_id: int) -> Optional[Dict[str, Any]]:
        """TennisApi getMatchBettingOdds: /api/tennis/event/{event_id}/odds."""
        path = f"/api/tennis/event/{match_id}/odds"
        try:
            return self._request_json("GET", path)
        except Exception as exc:
            logger.info("TennisApi betting odds unavailable. match_id=%s error=%s", match_id, exc)
            return None

    def get_match_featured_odds(self, match_id: int) -> Optional[Dict[str, Any]]:
        """Late fallback for featured odds candidate paths."""
        paths = [
            f"/api/tennis/event/{match_id}/odds/featured",
            f"/api/tennis/event/{match_id}/featured-odds",
            f"/api/tennis/event/{match_id}/odds/1/featured",
        ]
        for path in paths:
            try:
                return self._request_json("GET", path)
            except Exception as exc:
                logger.debug("TennisApi featured odds candidate failed. path=%s error=%s", path, exc)
        return None
