...
class TennisApiClient:
    ...
    def get_calendar_categories(
        ...
        """Tennis API calendar categories for a specific date.

        Correct replacement for retired flat endpoint
        /api/tennis/events/{day}/{month}/{year}.

        Confirmed flow from API support:
          1) /api/tennis/calendar/{day}/{month}/{year}/categories
          2) /api/tennis/category/{categoryId}/events/{day}/{month}/{year}
        """
        ...
        if isinstance(data, dict) and isinstance(data.get("category"), dict):
