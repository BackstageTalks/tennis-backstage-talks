...
class TennisApiClient:
    ...
    def get_match_betting_odds(self, match_id: int) -> Optional[Dict[str, Any]]:
        ...
        Confirmed RapidAPI UI endpoint returns a payload with markets/choices,
        typically including:
    ...
    def get_match_featured_odds(self, match_id: int) -> Optional[Dict[str, Any]]:
        """
        TennisApi getMatchFeaturedOdds.

        Featured odds are used only as a late PRO fallback after daily odds,
        winning odds, betting odds and all odds for event.

        Candidate paths are intentionally conservative because RapidAPI naming
        differs across TennisApi docs/snippets.
        """
        paths = [
