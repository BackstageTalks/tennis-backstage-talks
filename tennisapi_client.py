...
class TennisApiClient:
    ...
    def get_match_betting_odds(self, match_id: int) -> Optional[Dict[str, Any]]:
        ...
        Confirmed by RapidAPI UI: GET getMatchBettingOdds with path param id.
        Working REST path for markets is /api/tennis/event/{event_id}/odds.
        """
        paths = [
            f"/api/tennis/event/{match_id}/odds",
            f"/api/tennis/event/{match_id}/betting-odds",
        ]
        for path in paths:
            try:
                return self._request_json("GET", path)
            except Exception as exc:
                logger.debug("TennisApi betting odds candidate failed. path=%s error=%s", path, exc)
        return None

    def get_match_featured_odds(self, match_id: int) -> Optional[Dict[str, Any]]:
        """TennisApi getMatchFeaturedOdds best-effort fallback."""
        paths = [
