...
class TennisApiClient:
    ...
    def get_match_betting_odds(self, match_id: int) -> Optional[Dict[str, Any]]:
        """
        TennisApi getMatchBettingOdds.

        Confirmed endpoint from RapidAPI UI:
            /api/tennis/event/{event_id}/odds

        This endpoint returns markets/choices. The winner market is normally:
            marketName = Full time
            marketGroup = Home/Away
            marketPeriod = Match
            choices name 1 and 2 with fractionalValue.
        """
        ...
        try:
            ...
            logger.info(
                "TennisApi betting odds unavailable. match_id=%s error=%s",
                match_id,
                exc,
            )
            return None
    ...
    def get_match_featured_odds(self, match_id: int) -> Optional[Dict[str, Any]]:
        """
        TennisApi getMatchFeaturedOdds candidate paths.

        Used only after daily odds, winning odds, betting odds and all odds
        for event fail. Some TennisApi deployments expose this under slightly
        different paths, so candidates are tried conservatively.
        """
        paths = [
        ...
        for path in paths:
            try:
                ...
                logger.debug(
                    "TennisApi featured odds candidate failed. path=%s error=%s",
                    path,
                    exc,
                )
