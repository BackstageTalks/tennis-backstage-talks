...
def bst_sort_value(prediction):
    bst_probability = safe_float(
...
def has_bst_model_data(prediction):
...
def derive_bst_page_predictions(all_predictions):
    eligible = []

    for prediction in all_predictions:
        if not has_bst_model_data(prediction):
            continue
        ...
        updated["corq_display_probability"] = prediction.get("probability")

        # IMPORTANT:
        # The /BsT/ page is a BsT-view test page. The renderer uses
        # "probability" for the WIN % column and summary average.
        # Therefore we intentionally set probability to BsT AI probability
        # only for this derived page, without changing the original ALL data.
        updated["probability"] = bst_probability

        updated["top_mode"] = "BST_AI_SORT"
        updated["top_reason"] = "sorted and displayed by BsT AI probability"
...
def build_pages():
    ...
    bst_predictions = derive_bst_page_predictions(
        all_predictions,
    )
    ...
    write_page(
        ...
        title=SITE_TITLE,
        subtitle="BsT AI sorted tennis predictions",
    ...
    print("BST COUNT:", len(bst_predictions))
