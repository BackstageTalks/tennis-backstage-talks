...
def fetch_event_winning_odds_item(
    ...
    Order: getMatchWinningOdds -> getMatchBettingOdds -> getAllOddsForEvent
    -> getMatchFeaturedOdds.
    """
    attempts = [
        ("TennisApiMatchWinningOdds", lambda: client.get_match_winning_odds(event_id, provider_id()), normalize_winning_odds),
        ("TennisApiMatchBettingOdds", lambda: client.get_match_betting_odds(event_id), normalize_event_market_odds_payload),
        ("TennisApiAllOddsForEvent", lambda: client.get_all_odds_for_event(event_id, provider_id()), normalize_event_market_odds_payload),
        ("TennisApiMatchFeaturedOdds", lambda: client.get_match_featured_odds(event_id), normalize_event_market_odds_payload),
    ]
    for source_name, fetcher, normalizer in attempts:
        try:
            ...
            if normalized:
                return legacy_item_from_normalized_event_odds(event, normalized, source_name=source_name)
        except Exception as exc:
            logger.debug("Event odds fallback failed source=%s event_id=%s error=%s", source_name, event_id, exc)
    return None
