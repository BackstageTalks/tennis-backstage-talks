...
# ----------------------------------------------------------------------
# Dynamic category discovery override
# ----------------------------------------------------------------------


def _extract_category_id(category: Dict[str, Any]) -> Optional[int]:
    if not isinstance(category, dict):
        return None
    for key in ("id", "categoryId", "cid", "Cid"):
        ...
        try:
            ...
            continue
    # Some APIs nest the category.
    nested = category.get("category")
    if isinstance(nested, dict):
        return _extract_category_id(nested)
    return None


def get_active_category_ids_for_date(
    target_date: Optional[datetime] = None,
    force_refresh: bool = False,
) -> List[int]:
    target_date = betting_day_datetime(target_date)
    path = cache_path("calendar_categories", target_date)
        ...
        if isinstance(cached, list):
            ...
            for item in cached:
                if isinstance(item, dict):
                    category_id = _extract_category_id(item)
    ...
    try:
        client = TennisApiClient()
        categories = client.get_calendar_categories(
            day=target_date.day,
            month=target_date.month,
            year=target_date.year,
        )
        ...
        ids = sorted(set(ids))
        if ids:
            print("TennisApi active categories:", ids, str(path))
            return ids
    except Exception as exc:
        logger.warning(
    ...
    # Safe fallback for compatibility, but dynamic endpoint should normally work.
    return parse_category_ids()


# Override original get_events_for_date with dynamic-category discovery.
def get_events_for_date(
    target_date: Optional[datetime] = None,
    force_refresh: bool = False,
    category_ids: Optional[List[int]] = None,
    ...
    client = TennisApiClient()
    category_ids = category_ids or get_active_category_ids_for_date(
    ...
    for category_id in category_ids:
        try:
            ...
            logger.warning(
                "TennisApi dynamic category events fetch failed category_id=%s date=%s error=%s",
                category_id,
                target_date.date(),
                exc,
            )
