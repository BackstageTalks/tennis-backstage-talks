...
def normalize_event_market_odds_payload(raw_odds: Any) -> Optional[Dict[str, Any]]:
    """Normalize market odds payloads returned by getMatchBettingOdds/getAllOddsForEvent/getMatchFeaturedOdds."""
    ...
    for choice in choices:
        if not isinstance(choice, dict):
            continue
        name = str(choice.get("name") or "").strip()
        if name == "1" and choice1 is None:
            choice1 = choice
        elif name == "2" and choice2 is None:
            choice2 = choice
...
def fetch_event_winning_odds_item(
    ...
    """Try all useful TennisApi PRO event-level odds endpoints in priority order."""
    attempts: List[str] = []
    ...
    try:
        ...
        if normalized:
            item = legacy_item_from_normalized_event_odds(event, normalized, source_name="TennisApiMatchWinningOdds")
            if item:
                return item
        attempts.append("getMatchWinningOdds:no_data")
    except Exception as exc:
        attempts.append(f"getMatchWinningOdds:error:{exc}")

    try:
        event_odds_payload = client.get_match_betting_odds(event_id)
        normalized = normalize_event_market_odds_payload(event_odds_payload)
        if normalized:
            item = legacy_item_from_normalized_event_odds(event, normalized, source_name="TennisApiMatchBettingOdds")
            if item:
                return item
        attempts.append("getMatchBettingOdds:no_data")
    except Exception as exc:
        attempts.append(f"getMatchBettingOdds:error:{exc}")

    try:
        event_odds_payload = client.get_all_odds_for_event(event_id, provider_id())
        normalized = normalize_event_market_odds_payload(event_odds_payload)
        if normalized:
            item = legacy_item_from_normalized_event_odds(event, normalized, source_name="TennisApiAllOddsForEvent")
            if item:
                return item
        attempts.append("getAllOddsForEvent:no_data")
    except Exception as exc:
        attempts.append(f"getAllOddsForEvent:error:{exc}")

    try:
        event_odds_payload = client.get_match_featured_odds(event_id)
        normalized = normalize_event_market_odds_payload(event_odds_payload)
        if normalized:
            item = legacy_item_from_normalized_event_odds(event, normalized, source_name="TennisApiMatchFeaturedOdds")
            if item:
                return item
        attempts.append("getMatchFeaturedOdds:no_data")
    except Exception as exc:
        attempts.append(f"getMatchFeaturedOdds:error:{exc}")

    logger.debug("Event odds fallback failed event_id=%s attempts=%s", event_id, " | ".join(attempts))
    return None
...
def legacy_item_from_normalized_event_odds(
    event: Dict[str, Any],
    normalized: Dict[str, Any],
    source_name: str = "TennisApiEventOdds",
) -> Optional[Dict[str, Any]]:
    event = extract_event_from_payload(event) or event
    player1, player2 = event_home_away(event)
    event_id = event.get("id")
