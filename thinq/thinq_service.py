...
class ThinqService:
    ...
    def build_match_features(
        ...
        loader_errors = raw.get("loader_import_errors", {}) if isinstance(raw.get("loader_import_errors"), dict) else {}
        for layer_name in loader_errors:
            flags.append(f"{str(layer_name).upper()}_IMPORT_ERROR_NON_BLOCKING")
        h2h_ctx = raw.get("h2h", {}) if isinstance(raw.get("h2h"), dict) else {}
        if h2h_ctx.get("h2h_status") == "NO_DATA":
            flags.append("H2H_NO_DATA")
