...
try:
    ...
    try:
        from thinq.loaders.sackmann_loader import SackmannLoader
    except Exception:
        SackmannLoader = None
...
class H2HLoader:
    ...
    def load_h2h(
        ...
        **kwargs: Any,
        ...
        if self.use_cache and not force_refresh:
            ...
            if cached:
                cached.setdefault("h2h_status", "AVAILABLE" if cached.get("h2h_total_matches") else "NO_DATA")
                return cached
    ...
    def _load_from_sackmann(
        ...
        if self.sackmann_loader is not None:
            loader = self.sackmann_loader
        elif SackmannLoader is not None:
            loader = SackmannLoader()
        else:
            return self._empty_result(player1, player2, surface, reason="SackmannLoader unavailable", api_error=api_error)
    ...
    def _empty_result(self, player1: str, player2: str, surface: Optional[str], reason: str, api_error: Optional[str] = None) -> Dict[str, Any]:
        return {
            "player1": player1,
            "player2": player2,
            "surface": surface,
