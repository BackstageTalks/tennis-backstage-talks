...
This adapter attaches THINQ intelligence data to an existing CORQ prediction record.
It is deliberately defensive:
- If THINQ fails, CORQ output does not fail.
- THINQ does not become a standalone model.
- CORQ remains the final decision/output model.

Main responsibilities:
- call ThinqService.build_match_features(...)
- flatten THINQ edges/context into prediction fields
- optionally apply a small, capped THINQ adjustment to CORQ probability
"""
...
THINQ_APPLY_ADJUSTMENT = os.getenv("CORQ_THINQ_APPLY_ADJUSTMENT", "1") != "0"
...
def infer_tour_type(match: Dict[str, Any]) -> Optional[str]:
    value = str(match.get("gender") or match.get("tour_type") or match.get("type") or "").strip().lower()
    if value in ["atp", "m", "men", "male"]:
        return "atp"
    if value in ["wta", "f", "women", "female"]:
        return "wta"
    return None
...
def calculate_pick_adjustment(thinq_result: Dict[str, Any], prediction: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Winner-direction edges only. Match Dynamics stays visible as context but should not overdrive winner selection.
    weights = {
        "elo_edge": 0.45,
...
def flatten_thinq_fields(thinq_result: Dict[str, Any], prediction: Dict[str, Any]) -> Dict[str, Any]:
    ...
    data_quality = thinq_result.get("data_quality") if isinstance(thinq_result.get("data_quality"), dict) else {}
    ...
    output = {
        ...
        "thinq_data_quality": data_quality,

        # Flat edge fields for renderers, RSS, Telegram and diagnostics.
    ...
    # Backward/display compatibility: fill selected display fields only if old sets model did not provide them.
...
def attach_thinq_to_prediction(
    ...
    try:
        ...
        flat = flatten_thinq_fields(thinq_result, item)
        item.update(flat)

        adjustment_info = calculate_pick_adjustment(thinq_result, item)
        item["corq_thinq_adjustment"] = adjustment_info["adjustment"]
        item["corq_thinq_raw_adjustment"] = adjustment_info["raw_adjustment"]
        item["corq_thinq_confidence_multiplier"] = adjustment_info["confidence_multiplier"]
        item["corq_thinq_contributions"] = adjustment_info["contributions"]
        ...
        if THINQ_APPLY_ADJUSTMENT and current_probability is not None:
            adjusted_probability = clamp(
                current_probability + adjustment_info["adjustment"],
                THINQ_MIN_PROBABILITY,
                THINQ_MAX_PROBABILITY,
            )
            item["corq_raw_probability"] = round(current_probability, 4)
            item["corq_thinq_adjusted_probability"] = round(adjusted_probability, 4)
            item["probability"] = round(adjusted_probability, 4)
            item["corq_thinq_adjustment_applied"] = True

            # Keep DATA AI / TOP sorting consistent with final CORQ probability.
            if item.get("corq_ai_probability") is not None:
                item["corq_ai_probability_before_thinq"] = item.get("corq_ai_probability")
            item["corq_ai_probability"] = round(adjusted_probability, 4)
        else:
        ...
        item["thinq_flags"] = list(item["thinq_flags"]) + ["THINQ_ATTACH_FAILED"]
        return item
