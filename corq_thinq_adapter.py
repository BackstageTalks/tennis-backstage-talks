"""
CORQ -> THINQ Adapter v3

Purpose:
- THINQ is the intelligence layer.
- CORQ remains the final model/ranking owner.
- This adapter attaches THINQ intelligence output to every prediction record.
- If THINQ fails, the prediction still survives with explicit thinq_error fields.
"""
...
def build_thinq_features(match: Dict[str, Any], surface: Optional[str]) -> Dict[str, Any]:
    ...
    return service.build_match_features(
        ...
        level=match.get("level") or match.get("category") or match.get("tournament"),
...
def h2h_status_from_context(raw_h2h: Dict[str, Any]) -> str:
    if not isinstance(raw_h2h, dict) or not raw_h2h:
        return "NO_DATA"
    total = safe_float(raw_h2h.get("h2h_total_matches") or raw_h2h.get("total_matches") or raw_h2h.get("h2h_api_total_matches") or raw_h2h.get("h2h_sackmann_total_matches"))
    if total is not None and total > 0:
        return "AVAILABLE"
    status = str(raw_h2h.get("h2h_status") or raw_h2h.get("status") or "").upper()
    if status in {"AVAILABLE", "OK", "FOUND"}:
        return "AVAILABLE"
    return "NO_DATA"
...
def calculate_pick_adjustment(thinq_result: Dict[str, Any], prediction: Dict[str, Any]) -> Dict[str, Any]:
    ...
    confidence = safe_float(thinq_result.get("confidence")) or 0.0
    weights = {
        "elo_edge": 0.45,
        "surface_form_edge": 0.25,
        "recent_form_edge": 0.15,
        "level_form_edge": 0.10,
        "level_context_edge": 0.10,
        "h2h_edge": 0.08,
        "ta_edge": 0.08,
        "opponent_quality_edge": 0.06,
        "fatigue_edge": 0.06,
...
def flatten_thinq_fields(thinq_result: Dict[str, Any], prediction: Dict[str, Any]) -> Dict[str, Any]:
    ...
    raw_h2h = raw.get("h2h") if isinstance(raw.get("h2h"), dict) else {}
    ...
    output = {
        ...
        "thinq_elo_edge": elo_edge,
        "thinq_surface_form_edge": edges.get("surface_form_edge"),
        "thinq_recent_form_edge": edges.get("recent_form_edge"),
        "thinq_level_form_edge": edges.get("level_form_edge") or edges.get("level_context_edge"),
        "thinq_h2h_edge": edges.get("h2h_edge"),
        "thinq_ta_edge": edges.get("ta_edge"),
        "thinq_opponent_quality_edge": edges.get("opponent_quality_edge"),
        ...
        "thinq_h2h_status": h2h_status_from_context(raw_h2h),
        "thinq_h2h_source": raw_h2h.get("h2h_source") or raw_h2h.get("source"),
        "thinq_h2h_total_matches": raw_h2h.get("h2h_total_matches") or raw_h2h.get("total_matches") or raw_h2h.get("h2h_api_total_matches") or raw_h2h.get("h2h_sackmann_total_matches"),
        "thinq_h2h_confidence": raw_h2h.get("h2h_confidence"),
        "thinq_elo_pick": None,
    }

    if safe_float(elo_edge) is not None:
        if safe_float(elo_edge) > 0:
            output["thinq_elo_pick"] = prediction.get("player1")
        elif safe_float(elo_edge) < 0:
            output["thinq_elo_pick"] = prediction.get("player2")

    # Renderer compatibility aliases.
    output["thinq_conf"] = confidence
    output["thinq_h2h_signal"] = output["thinq_h2h_status"]
    output["thinq_h2h"] = output["thinq_h2h_status"]
    ...
    if prediction.get("sets_probability") is not None and output.get("thinq_decider_probability") is None:
        output["thinq_decider_probability"] = prediction.get("sets_probability")
...
def attach_thinq_to_prediction(
    ...
    try:
        ...
        print(
            ...
            "vs",
            item.get("player2"),
            "conf=", item.get("thinq_confidence"),
            "elo=", item.get("thinq_elo_edge"),
            "h2h=", item.get("thinq_h2h_status"),
            "adj=", item.get("corq_thinq_adjustment"),
        )
        return item
        ...
        item["thinq_confidence"] = None
        item["thinq_elo_pick"] = None
        item["thinq_h2h_status"] = "ERROR"
        print("THINQ ATTACH ERROR:", item.get("player1"), "vs", item.get("player2"), str(exc))
        return item
