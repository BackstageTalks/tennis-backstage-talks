...
def resolve_ai_metrics(prediction):
    ...
    blend_pct = normalize_probability_for_display(
        prediction.get("blend_probability")
    )
...
def render_data_ai_box(prediction, model_view="corq"):
    ...
    blend_display = format_pct_points(metrics["blend_pct"])
    ...
    if model_view == "blend":
        rows = (
            metric_row("Blend", blend_display)
...
.nav {{ display:flex; gap:18px; align-items:center; flex-wrap:wrap; padding-top:8px; white-space:nowrap; }}
...
td {{ padding:16px 12px; border-bottom:1px solid var(--border); vertical-align:top; font-size:13px; line-height:1.2; }}
tr:hover {{ background:rgba(255,255,255,.03); }}
.rank {{ font-size:15px; line-height:1.15; font-weight:900; color:var(--blue); }}
.pick-name {{ font-size:22px; line-height:1.08; font-weight:900; letter-spacing:.01em; }}
.pick-sub {{ color:var(--green); font-size:15px; line-height:1.05; margin-top:3px; font-weight:900; }}
.match-name {{ color:var(--muted); font-size:12px; line-height:1.15; margin-top:7px; }}
.match-meta {{ color:var(--blue); font-size:12px; line-height:1.15; margin-top:4px; font-weight:800; }}
.probability {{ font-size:15px; line-height:1.15; font-weight:900; color:var(--green); }}
.odds {{ font-size:15px; line-height:1.15; font-weight:900; color:var(--yellow); }}
...
@media (max-width:1050px) {{ .header{{display:block;}} .logo{{white-space:normal;}} .summary{{grid-template-columns:1fr 1fr;}} }}
...
def render_rss(predictions, title, link):
    ...
    for prediction in predictions:
        ...
        blend_ai = format_pct_points(metrics["blend_pct"])
