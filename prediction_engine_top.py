...
def has_both_model_outputs(prediction: Dict[str, Any]) -> bool:
    """
    Production pick rule:
    A public TOP/Telegram pick must have usable outputs from both models.

    Corq-only or Thinq-missing rows are allowed only in ALL/debug views.
    """
...
def apply_top7_metadata(prediction: Dict[str, Any]) -> Dict[str, Any]:
    ...
    item["top_reason"] = (
        ...
        "both Corq and Thinq probabilities required, CorqQ/ThinqQ 80/20 "
        "cross influence, model_gap <= 15%."
    )
...
def get_top_predictions(
    ...
    - Public TOP requires both Corq and Thinq model output.
    """
