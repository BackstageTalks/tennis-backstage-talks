...
# ----------------------------------------------------------------------
# 2026-07 clean architecture override
# ----------------------------------------------------------------------
# CORQ owns ranking. TOP7 is only the first TOP_N records from the CORQ-ranked
# output. TOP7 must not directly read H2H or individual THINQ/MARQ layers.
...
def get_corq_ranking_score(prediction: Dict[str, Any]) -> Optional[float]:
...
def has_corq_ranking_output(prediction: Dict[str, Any]) -> bool:
    return get_corq_ranking_score(prediction) is not None
...
def ensure_corq_ranking_metadata(prediction: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """Return a TOP7-ready record without reading individual THINQ layers.

    Preferred path: use CORQ fields already present in ALL:
    - corq_adjusted_score / corq_top_score
    - corq_q_probability / corq_thinq_adjusted_probability / corq_ai_probability

    Fallback path: if old ALL lacks explicit CORQ ranking fields, use the legacy
    CORQ direct metadata builder. This fallback only uses already-flattened CORQ
    and Thinq AI probabilities, never H2H/ELO/individual THINQ internals.
    """
    ...
    item["top_reason"] = (
        "CORQ ranked all eligible matches using finalized CORQ output; "
        "TOP7 is only the first TOP_N records from that CORQ ranking."
    )
    return item


def get_corq_rank_tuple(prediction: Dict[str, Any]) -> Tuple[float, float, float]:
    return (
        safe_float(prediction.get("corq_adjusted_score")) or 0.0,
        safe_float(prediction.get("corq_q_probability")) or 0.0,
        safe_float(prediction.get("odds")) or 0.0,
    )
...
def get_model_top_predictions(
    ...
    # Thinq and Cloq pages keep their specialized views. They are not the CORQ TOP7.
    eligible = [
        apply_top7_metadata(prediction, model=model)
        for prediction in all_predictions
        if eligible_model_top7(prediction, model=model)
