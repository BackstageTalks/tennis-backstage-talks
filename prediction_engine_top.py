...
# ----------------------------------------------------------------------
# 2026-07 Corq/Cloq selector override for direct prediction workflows
# ----------------------------------------------------------------------
...
CLOQ_CLOSE_MAX_PICK_ODDS = float(os.getenv("CLOQ_CLOSE_MAX_PICK_ODDS", str(CLOQ_MAX_ODDS)))
CLOQ_CORQ_WEIGHT = float(os.getenv("CLOQ_CORQ_WEIGHT", "0.60"))
CLOQ_THINQ_WEIGHT = float(os.getenv("CLOQ_THINQ_WEIGHT", "0.40"))


def _corq_direct_selector_score(prediction: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    corq = get_corq_probability(prediction)
    odds = get_pick_odds(prediction)
    if corq is None or odds is None or odds < CORQ_SELECTOR_MIN_ODDS or odds > CORQ_SELECTOR_MAX_ODDS:
        return None
    thinq = get_thinq_probability(prediction)
    ai_match = get_ai_match(prediction)
    corq_q = 0.80 * corq + 0.20 * thinq if thinq is not None else corq
    market_break_even = 1.0 / odds if odds > 1.0 else 0.0
    edge = corq_q - market_break_even if market_break_even else 0.0
    bonus = min(max(edge, 0.0) * 0.25, 0.02)
    penalty = 0.0
    flags: List[str] = []
    if odds < 1.45:
        penalty += 0.10; flags.append("LOW_ODDS_LT_1_45")
    elif odds < 1.55:
        penalty += 0.08; flags.append("LOW_ODDS_LT_1_55")
    elif odds < 1.65:
        penalty += 0.05; flags.append("LOW_ODDS_LT_1_65")
    elif odds < 1.75:
        penalty += 0.02; flags.append("LOW_ODDS_LT_1_75")
    if thinq is None:
        penalty += 0.03; flags.append("THINQ_MISSING")
    if ai_match is None:
        penalty += 0.04; flags.append("AI_MATCH_MISSING")
    elif ai_match < 85:
        penalty += 0.05; flags.append("LOW_AI_MATCH_LT_85")
    elif ai_match < 90:
        penalty += 0.03; flags.append("LOW_AI_MATCH_LT_90")
    elif ai_match >= 95:
        bonus += 0.02; flags.append("AI_MATCH_ELITE")
    elif ai_match >= 90:
        bonus += 0.01; flags.append("AI_MATCH_GOOD")
    return {
        "corq": corq,
        "thinq": thinq,
        "corq_q": corq_q,
...
def _apply_corq_direct_metadata(prediction: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    score = _corq_direct_selector_score(prediction)
    if not score:
        return None
    item = dict(prediction)
    item["probability"] = round(score["corq_q"], 4)
    item["corq_raw_probability"] = round(score["corq"], 4)
    item["thinq_raw_probability"] = round(score["thinq"], 4) if score["thinq"] is not None else None
    item["corq_q_probability"] = round(score["corq_q"], 4)
    item["corq_top_score"] = round(score["score"], 4)
    item["corq_adjusted_score"] = round(score["score"], 4)
    item["corq_bonus"] = round(score["bonus"], 4)
    item["corq_edge_bonus"] = round(score["bonus"], 4)
    item["corq_risk_penalty"] = round(score["penalty"], 4)
    item["corq_risk_flags"] = score["flags"]
    item["model_view"] = MODEL_CORQ
    item["top_mode"] = "TOP7_CORQ_RISK_ADJUSTED"
    item["top_reason"] = "Corq TOP7: main Corq model with risk-adjusted ranking; Thinq is a soft sanity signal only."
    return item
...
def get_model_top_predictions(
    ...
    if model == MODEL_CORQ:
        eligible = []
        for prediction in all_predictions:
            item = _apply_corq_direct_metadata(prediction)
            if item:
        ...
        selected = eligible[:TOP_N]
        print("TOP7 CORQ COUNT:", len(selected))
        for idx, item in enumerate(selected, start=1):
            print("TOP7 CORQ PICK:", idx, item.get("pick"), "vs", item.get("opponent"), "odds=", item.get("odds"), "score=", item.get("corq_top_score"), "flags=", item.get("corq_risk_flags"))
        return selected

    # Keep legacy direct selectors for Thinq and Cloq. Build pages contains the cleaner Cloq close-odds renderer.
    eligible = [
        apply_top7_metadata(prediction, model=model)
        for prediction in all_predictions
        if eligible_model_top7(prediction, model=model)
...
# ----------------------------------------------------------------------
# 2026-07 production guard override
# ----------------------------------------------------------------------
# Production CORQ/TOP7 must not leak doubles and must not publish records where
# the Thinq/BsT probability side is missing. Doubles stay in ALL only until a
# dedicated DOUQ model exists.

DOUBLES_SEPARATORS = [" / ", "/", " & ", " + ", " and "]
...
def is_doubles_side(value: Any) -> bool:
    ...
    if not text:
        return False
    lower = text.lower()
    return any(separator in lower for separator in DOUBLES_SEPARATORS)


def is_doubles_prediction(prediction: Dict[str, Any]) -> bool:
    if str(prediction.get("match_type") or prediction.get("event_type") or "").lower() == "doubles":
        return True
    if is_doubles_side(prediction.get("player1")) or is_doubles_side(prediction.get("player2")):
        return True
    if is_doubles_side(prediction.get("pick")) or is_doubles_side(prediction.get("opponent")):
        return True
    match_text = str(prediction.get("match") or "")
    if " / " in match_text and " vs " in match_text.lower():
        return True
    return False


def has_required_corq_top_outputs(prediction: Dict[str, Any]) -> bool:
    # TOP7 is an output from CORQ ranking, but production still requires both
    # model-side probabilities and AI Match to avoid publishing no-Thinq records.
    if get_corq_probability(prediction) is None:
        return False
    if get_thinq_probability(prediction) is None:
        return False
...
def is_corq_rank_eligible(prediction: Dict[str, Any]) -> bool:
    if is_doubles_prediction(prediction):
        return False
    if not has_required_corq_top_outputs(prediction):
        return False
    odds = get_pick_odds(prediction)
    if odds is None or odds < CORQ_SELECTOR_MIN_ODDS or odds > CORQ_SELECTOR_MAX_ODDS:
        return False
    return has_corq_ranking_output(prediction)


def ensure_corq_ranking_metadata(prediction: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    # First reject doubles/no-Thinq records before any fallback can resurrect them.
    if is_doubles_prediction(prediction):
        return None
    if not has_required_corq_top_outputs(prediction):
        return None

    if not is_corq_rank_eligible(prediction):
        fallback = _apply_corq_direct_metadata(prediction)
        if not fallback or not is_corq_rank_eligible(fallback):
            return None
        item = fallback
    else:
        item = dict(prediction)

    score = get_corq_ranking_score(item)
    if score is None:
        return None

    corq_q = safe_float(item.get("corq_q_probability"))
    if corq_q is None:
        corq_q = safe_float(item.get("corq_thinq_adjusted_probability"))
    if corq_q is None:
        corq_q = safe_float(item.get("corq_ai_probability"))
    if corq_q is None:
        corq_q = safe_float(item.get("probability"))
    ...
    item["top_reason"] = (
        "CORQ ranked eligible singles with complete Corq/Thinq/AI Match outputs; "
        "TOP7 is only the first TOP_N records from that CORQ ranking."
    )
    return item
...
def get_model_top_predictions(
    ...
    if model == MODEL_CORQ:
        ...
        rejected_doubles = 0
        rejected_missing_thinq = 0
        for prediction in all_predictions:
            if is_doubles_prediction(prediction):
                rejected_doubles += 1
                continue
            if not has_required_corq_top_outputs(prediction):
                rejected_missing_thinq += 1
                continue
            item = ensure_corq_ranking_metadata(prediction)
            if item:
                ranked_candidates.append(item)

        ranked_candidates = deduplicate_predictions(ranked_candidates)
        ranked_candidates.sort(key=get_corq_rank_tuple, reverse=True)
        selected = ranked_candidates[:TOP_N]

        print("CORQ RANKED CANDIDATES:", len(ranked_candidates))
        print("CORQ REJECTED DOUBLES:", rejected_doubles)
        print("CORQ REJECTED MISSING THINQ:", rejected_missing_thinq)
        print("TOP7 FROM CORQ RANKING COUNT:", len(selected))
        for idx, item in enumerate(selected, start=1):
            print(
                "TOP7 CORQ RANK:",
                idx,
    ...
    # Thinq and Cloq pages keep specialized views, but also exclude doubles from
    # production pages until DOUQ exists.
    eligible = []
    for prediction in all_predictions:
        if is_doubles_prediction(prediction):
            continue
        if not eligible_model_top7(prediction, model=model):
            continue
        eligible.append(apply_top7_metadata(prediction, model=model))
    eligible = deduplicate_predictions(eligible)
    eligible.sort(key=lambda item: model_top7_score(item, model=model), reverse=True)
    return eligible[:TOP_N]
