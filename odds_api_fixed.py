...
def _to_legacy_odds_item(odds: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "player1": odds.get("player1") or odds.get("home") or odds.get("home_team"),
        "player2": odds.get("player2") or odds.get("away") or odds.get("away_team"),
        "home_odds": p1,
        "away_odds": p2,
        "p1_odds": p1,
        "p2_odds": p2,
...
def _find_in_legacy_odds_list(
    ...
    """Find odds by normalized player names, independent of player order.

    Handles diacritics, initials, punctuation and reversed home/away ordering.
    """
    ...
    requested_pair = _pair_key(player1, player2)
    ...
    for item in odds_list:
        ...
        if not norm_item_p1 or not norm_item_p2:
            continue
        ...
        pair_match = requested_pair == _pair_key(item_p1, item_p2)

        if direct or pair_match:
            matched = dict(item)
            matched["odds_match_method"] = "PLAYER_PAIR_DIRECT" if direct else "PLAYER_PAIR_KEY"
            return matched
        ...
        if reversed_match:
            ...
            swapped["odds1"] = p1
            swapped["odds2"] = p2
            swapped["odds_match_method"] = "PLAYER_PAIR_REVERSED"
            return swapped
    return None


def _item_player1(item: Dict[str, Any]) -> Any:
    return item.get("player1") or item.get("home_team") or item.get("homeTeam") or item.get("home") or item.get("home_name") or item.get("participant1")


def _item_player2(item: Dict[str, Any]) -> Any:
    return item.get("player2") or item.get("away_team") or item.get("awayTeam") or item.get("away") or item.get("away_name") or item.get("participant2")
...
def _pair_key(player1: Any, player2: Any) -> str:
    return "|".join(sorted([_compact_name(player1), _compact_name(player2)]))
...
def _name_keys(normalized: str) -> set:
    ...
    if parts:
        keys.add(parts[-1])
        keys.add(_compact_name(parts[-1]))
    if len(parts) >= 2:
...
def _names_match_normalized(a: str, b: str) -> bool:
    ...
    if a == b:
        return True
    if _compact_name(a) == _compact_name(b):
        return True
