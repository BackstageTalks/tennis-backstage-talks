...
def calculate_blenq_risk(
    ...
    if model_gap is not None and model_gap > TOP_MAX_MODEL_GAP:
    ...
    for prediction in all_predictions:
        ...
        eligible.append(
            apply_consensus_metadata(
                ...
                "CORQ_TOP7_CONSENSUS",
                "CorqQ = 80% Corq + 20% Thinq; both model probabilities required; model_gap <= 15%; odds >= 1.40.",
            )
        )
...
def derive_thinq_predictions(all_predictions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for prediction in all_predictions:
        ...
        updated = apply_consensus_metadata(
            ...
            "ThinqQ = 80% Thinq + 20% Corq; both model probabilities required; model_gap <= 15%; odds >= 1.40.",
        )
...
def eligible_blenq(prediction: Dict[str, Any]) -> Optional[Dict[str, float]]:
    ...
    if odds1 is None or odds2 is None:
        return None
    if scores["corq_probability"] < BLENQ_MIN_CORQ:
        return None
    if scores["thinq_probability"] < BLENQ_MIN_THINQ:
        return None
