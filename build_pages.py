...
def get_thinq_probability(prediction: Dict[str, Any]) -> Optional[float]:
    return normalize_probability_decimal(prediction.get("bst_ai_probability") or prediction.get("thinq_ai_probability"))
...
def has_both_model_outputs(prediction: Dict[str, Any]) -> bool:
    return get_corq_probability(prediction) is not None and get_thinq_probability(prediction) is not None and get_ai_match(prediction) is not None
...
def calculate_market_fair_probabilities(odds_player1: Optional[float], odds_player2: Optional[float]) -> Tuple[Optional[float], Optional[float], Optional[float]]:
    if odds_player1 is None or odds_player2 is None or odds_player1 <= 1.0 or odds_player2 <= 1.0:
...
def calculate_pick_market_fair(prediction: Dict[str, Any], fair1: Optional[float], fair2: Optional[float]) -> Optional[float]:
...
def calculate_blenq_tier(blenq_probability: float, edge_pp: float, ai_match: float, marq_signal: Any) -> str:
    signal = str(marq_signal or "").upper().strip()
    negative_market = signal in {"CAUTION", "BEARISH"}
    if blenq_probability >= 0.62 and edge_pp >= 0.05 and ai_match >= 92 and not negative_market:
        return "BLENQ_A"
    if blenq_probability >= 0.60 and edge_pp >= 0.03 and ai_match >= 90:
        return "BLENQ_B"
    return "BLENQ_WATCH"


def calculate_blenq_risk(prediction: Dict[str, Any], market_gap: Optional[float], edge_pp: Optional[float], ai_match: Optional[float], model_gap: Optional[float]) -> str:
    notes = []
...
def unique_key(prediction: Dict[str, Any]) -> Tuple[str, str]:
    return (str(prediction.get("match_id") or prediction.get("event_id") or prediction.get("match") or ""), str(prediction.get("pick") or ""))
...
def apply_consensus_metadata(prediction: Dict[str, Any], display_probability: float, view_name: str, reason: str) -> Dict[str, Any]:
...
def mark_blenq_view(prediction: Dict[str, Any], blenq_probability: float, scores: Dict[str, float]) -> Dict[str, Any]:
    ...
    updated["blenq_tier"] = calculate_blenq_tier(blenq_probability, edge_pp or 0.0, ai_match or 0.0, prediction.get("marq_ai_signal"))
    updated["blenq_risk"] = calculate_blenq_risk(prediction, market_gap, edge_pp, ai_match, model_gap)
    return updated
...
def eligible_common_top(prediction: Dict[str, Any]) -> Optional[Dict[str, float]]:
    ...
    if not scores:
        return None
    if scores["corq_q_probability"] < TOP_MIN_CORQ_Q or scores["thinq_q_probability"] < TOP_MIN_THINQ_Q:
        return None
...
def derive_corq_predictions(all_predictions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for prediction in all_predictions:
        ...
        if not scores:
            continue
        eligible.append(apply_consensus_metadata(prediction, scores["corq_q_probability"], "CORQ_TOP7_CONSENSUS", "CorqQ = 80% Corq + 20% Thinq; both models required; model_gap <= 15%; odds >= 1.40."))
    eligible = dedupe_predictions(eligible)
    eligible.sort(key=lambda item: (safe_float(item.get("consensus_score")) or 0.0, safe_float(item.get("corq_q_probability")) or 0.0, safe_float(item.get("ai_match")) or 0.0), reverse=True)
...
def derive_thinq_predictions(all_predictions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for prediction in all_predictions:
        ...
        if not scores:
            continue
        updated = apply_consensus_metadata(prediction, scores["thinq_q_probability"], "THINQ_TOP7_CONSENSUS", "ThinqQ = 80% Thinq + 20% Corq; both models required; model_gap <= 15%; odds >= 1.40.")
        updated["bst_ai_probability"] = scores["thinq_q_probability"]
        eligible.append(updated)
    eligible = dedupe_predictions(eligible)
    eligible.sort(key=lambda item: (safe_float(item.get("consensus_score")) or 0.0, safe_float(item.get("thinq_q_probability")) or 0.0, safe_float(item.get("ai_match")) or 0.0), reverse=True)
    return eligible[:TOP_N]
...
def eligible_blenq(prediction: Dict[str, Any]) -> Optional[Dict[str, float]]:
    ...
    odds1 = get_odds_player1(prediction)
    odds2 = get_odds_player2(prediction)
    if odds is None or odds < BLENQ_MIN_PICK_ODDS or odds > BLENQ_MAX_PICK_ODDS:
        return None
    if scores["corq_probability"] < BLENQ_MIN_CORQ or scores["thinq_probability"] < BLENQ_MIN_THINQ:
        return None
    if scores["consensus_score"] < BLENQ_MIN_PROBABILITY:
        return None
    if scores["ai_match"] < BLENQ_MIN_AI_MATCH:
        return None
...
def derive_blenq_predictions(all_predictions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    eligible = dedupe_predictions(eligible)
    eligible.sort(key=lambda item: (safe_float(item.get("edge_pp")) or 0.0, safe_float(item.get("blenq_probability")) or 0.0, safe_float(item.get("ai_match")) or 0.0), reverse=True)
    return eligible
...
def print_prediction_sample(label: str, predictions: List[Dict[str, Any]]) -> None:
    ...
    for idx, item in enumerate(predictions[:10], start=1):
        print(idx, item.get("pick"), "vs", item.get("opponent"), "prob=", item.get("probability"), "odds=", item.get("odds"), "corq_q=", item.get("corq_q_probability"), "thinq_q=", item.get("thinq_q_probability"), "gap=", item.get("model_gap"), "consensus=", item.get("consensus_score"), "mode=", item.get("top_mode"))
...
def build_pages() -> None:
    ...
    save_json("public/blenq_predictions_latest.json", blenq_predictions)
    ...
    write_page(blenq_predictions, SITE_TITLE, "Blenq close-odds market-edge predictions", "public/Blend/index.html")
    write_rss(blenq_predictions, f"{SITE_TITLE} - Blenq", f"{BASE_URL}/Blend/", "public/tennis_blend.xml")
