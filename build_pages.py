...
THINQ_MIN_AI_MATCH = float(os.getenv("THINQ_MIN_AI_MATCH", "90.0"))
...
# Corq low-odds overconfidence guard.
# This is a ranking penalty, not a hard filter. It targets cases like 75%+ model probability,
# odds below 1.60, and a very large model edge versus fair market probability.
...
# Cloq is a close-odds / 50-50 value model. It can accept sub-50% model probability
# when the fair market probability is lower and the edge is large enough.
CLOQ_MIN_PROB_LOW_ODDS = float(os.getenv("CLOQ_MIN_PROB_LOW_ODDS", "0.55"))      # odds < 2.00
CLOQ_MIN_PROB_MID_ODDS = float(os.getenv("CLOQ_MIN_PROB_MID_ODDS", "0.50"))      # 2.00 - 2.49
CLOQ_MIN_PROB_HIGH_ODDS = float(os.getenv("CLOQ_MIN_PROB_HIGH_ODDS", "0.45"))    # 2.50 - 3.49
CLOQ_MIN_PROB_LONG_ODDS = float(os.getenv("CLOQ_MIN_PROB_LONG_ODDS", "0.40"))    # 3.50+
CLOQ_MIN_EDGE_LOW_ODDS = float(os.getenv("CLOQ_MIN_EDGE_LOW_ODDS", "0.03"))      # odds < 2.00
CLOQ_MIN_EDGE_MID_ODDS = float(os.getenv("CLOQ_MIN_EDGE_MID_ODDS", "0.04"))      # 2.00 - 2.49
CLOQ_MIN_EDGE_HIGH_ODDS = float(os.getenv("CLOQ_MIN_EDGE_HIGH_ODDS", "0.06"))    # 2.50 - 3.49
CLOQ_MIN_EDGE_LONG_ODDS = float(os.getenv("CLOQ_MIN_EDGE_LONG_ODDS", "0.08"))    # 3.50+
...
def calculate_quality_scores(prediction: Dict[str, Any]) -> Optional[Dict[str, float]]:
    ...
    corq_q_edge = corq_q - comparison_probability if comparison_probability is not None else None
    ...
    return {
        ...
        "corq_q_edge": corq_q_edge or 0.0,
...
def calculate_corq_adjusted_score(prediction: Dict[str, Any], scores: Dict[str, float]) -> Dict[str, Any]:
    """
    Corq ranking guard for low-odds overconfidence.

    This does not remove picks. It lowers the rank of cases where CorqQ is very high,
    odds are low, and CorqQ edge versus fair market probability is unusually large.
    """
    odds = get_prediction_odds(prediction)
    corq_q = safe_float(scores.get("corq_q_probability")) or 0.0
    corq_q_edge = safe_float(scores.get("corq_q_edge")) or 0.0
    ai_match = safe_float(scores.get("ai_match"))

    flags = []
    risk_penalty = 0.0

    if odds is not None and odds < CORQ_LOW_ODDS_GUARD_MAX_ODDS:
        if corq_q >= CORQ_LOW_ODDS_GUARD_MIN_Q and corq_q_edge >= CORQ_LOW_ODDS_GUARD_MIN_EDGE:
            flags.append("LOW_ODDS_OVERCONFIDENCE")
            risk_penalty += CORQ_LOW_ODDS_GUARD_PENALTY

            if corq_q_edge >= CORQ_LOW_ODDS_EXTREME_EDGE and (ai_match is None or ai_match < CORQ_LOW_ODDS_ELITE_AI_MATCH):
                flags.append("LOW_ODDS_EXTREME_EDGE_NOT_ELITE_AI_MATCH")
                risk_penalty += CORQ_LOW_ODDS_EXTREME_PENALTY

    edge_bonus = min(max(corq_q_edge, 0.0) * CORQ_EDGE_BONUS_WEIGHT, CORQ_EDGE_BONUS_CAP)
    adjusted_score = corq_q + edge_bonus - risk_penalty

    return {
        "corq_adjusted_score": adjusted_score,
        "corq_edge_bonus": edge_bonus,
        "corq_risk_penalty": risk_penalty,
        "corq_risk_flags": flags,
    }
...
def dynamic_cloq_min_probability(odds: Optional[float]) -> float:
    if odds is None:
        return min(CLOQ_MIN_PROBABILITY, CLOQ_MIN_PROB_MID_ODDS)
    if odds < 2.00:
        return CLOQ_MIN_PROB_LOW_ODDS
    if odds < 2.50:
        return CLOQ_MIN_PROB_MID_ODDS
    if odds < 3.50:
        return CLOQ_MIN_PROB_HIGH_ODDS
    return CLOQ_MIN_PROB_LONG_ODDS


def dynamic_cloq_required_edge(odds: Optional[float]) -> float:
    if odds is None:
        return CLOQ_MIN_EDGE
    if odds < 2.00:
        return max(CLOQ_MIN_EDGE, CLOQ_MIN_EDGE_LOW_ODDS)
    if odds < 2.50:
        return max(CLOQ_MIN_EDGE, CLOQ_MIN_EDGE_MID_ODDS)
    if odds < 3.50:
        return max(CLOQ_MIN_EDGE, CLOQ_MIN_EDGE_HIGH_ODDS)
    return max(CLOQ_MIN_EDGE, CLOQ_MIN_EDGE_LONG_ODDS)


def cloq_model_gap_bucket(model_gap: Optional[float]) -> str:
    """
    Cloq does not penalize model_gap yet. We only tag it for tracking.
    After enough results, compare ROI/hit-rate by bucket and decide whether to tighten Cloq.
    """
    if model_gap is None:
        return "NO_GAP_DATA"
    try:
        ...
        return "NO_GAP_DATA"
    if gap <= 0.07:
        return "CLOQ_LOW_GAP_0_07"
    if gap <= 0.10:
        return "CLOQ_MEDIUM_GAP_07_10"
    if gap <= 0.15:
        return "CLOQ_HIGH_GAP_10_15"
    return "CLOQ_CONFLICT_GT_15"
...
def apply_consensus_metadata(
    ...
    updated["corq_q_edge"] = round(scores.get("corq_q_edge", 0.0), 4)
...
def mark_cloq_view(
    ...
    updated["cloq_required_edge"] = round(dynamic_cloq_required_edge(odds), 4)
    updated["cloq_min_probability"] = round(dynamic_cloq_min_probability(odds), 4)
    updated["cloq_value_mode"] = "UNDERDOG_VALUE" if cloq_probability < 0.50 else "CLOSE_VALUE"
    updated["cloq_model_gap_bucket"] = cloq_model_gap_bucket(model_gap)
    updated["cloq_tracking_note"] = "model_gap is tracked only, not used as a hard Cloq filter yet"
...
def derive_corq_predictions(all_predictions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for prediction in all_predictions:
        ...
        updated = apply_consensus_metadata(
            ...
            "CORQ_TOP7_ADJUSTED_RANKING",
            "CorqQ = 80% Corq + 20% Thinq; low-odds overconfidence guard is used as ranking penalty, not hard filter.",
        )
        corq_rank = calculate_corq_adjusted_score(prediction, scores)
        updated["corq_adjusted_score"] = round(corq_rank["corq_adjusted_score"], 4)
        updated["corq_edge_bonus"] = round(corq_rank["corq_edge_bonus"], 4)
        updated["corq_risk_penalty"] = round(corq_rank["corq_risk_penalty"], 4)
        updated["corq_risk_flags"] = corq_rank["corq_risk_flags"]
        eligible.append(updated)

    eligible = dedupe_predictions(eligible)
    eligible.sort(
        key=lambda item: (
            safe_float(item.get("corq_adjusted_score")) or 0.0,
...
def eligible_cloq(prediction: Dict[str, Any]) -> Optional[Dict[str, float]]:
    ...
    if scores["thinq_probability"] < CLOQ_MIN_THINQ:
        return None
    cloq_min_probability = dynamic_cloq_min_probability(odds)
    if scores["consensus_score"] < cloq_min_probability:
        return None
    if scores["ai_match"] < CLOQ_MIN_AI_MATCH:
        return None

    # Cloq scans ALL predictions. model_gap is intentionally NOT a hard filter for Cloq now.
    # It is stored in cloq_model_gap_bucket and evaluated later from results.
    ...
    required_edge = dynamic_cloq_required_edge(odds)
    if edge_pp is None or edge_pp < required_edge:
        return None

    # Do not use THIN MARKET / NO SHARP DATA as a hard Cloq filter for now.
    # Current market dataset is often thin by default, so market quality is not yet a reliable discriminator.
    scores["cloq_required_edge"] = required_edge
    scores["cloq_min_probability"] = cloq_min_probability
    scores["cloq_edge"] = edge_pp
    scores["cloq_model_gap_bucket"] = cloq_model_gap_bucket(scores.get("model_gap"))
    return scores
...
def print_prediction_sample(label: str, predictions: List[Dict[str, Any]]) -> None:
    ...
    for idx, item in enumerate(predictions[:10], start=1):
        print(
            ...
            "cloq_gap_bucket=", item.get("cloq_model_gap_bucket"),
...
def build_pages() -> None:
    ...
    try:
        from prediction_engine_top import (
            get_corq_top_predictions,
            get_thinq_top_predictions,
            get_cloq_top_predictions,
        )
        corq_predictions = get_corq_top_predictions(all_predictions)
        thinq_predictions = get_thinq_top_predictions(all_predictions)
        cloq_predictions = get_cloq_top_predictions(all_predictions)
        print("BUILD PAGES SELECTOR:", "prediction_engine_top canonical selectors")
    except Exception as exc:
        print("BUILD PAGES SELECTOR ERROR, USING LOCAL FALLBACK:", str(exc))
    ...
    print("CLOQ MODEL GAP:", "tracking only, no hard filter")
    print("CLOQ VALUE MODE:", "prob", CLOQ_MIN_PROB_LOW_ODDS, CLOQ_MIN_PROB_MID_ODDS, CLOQ_MIN_PROB_HIGH_ODDS, CLOQ_MIN_PROB_LONG_ODDS, "edge", CLOQ_MIN_EDGE_LOW_ODDS, CLOQ_MIN_EDGE_MID_ODDS, CLOQ_MIN_EDGE_HIGH_ODDS, CLOQ_MIN_EDGE_LONG_ODDS)
    print("CORQ TOP7 COUNT:", len(corq_predictions))
    print("CORQ LOW ODDS GUARD:", "odds<", CORQ_LOW_ODDS_GUARD_MAX_ODDS, "q>=", CORQ_LOW_ODDS_GUARD_MIN_Q, "edge>=", CORQ_LOW_ODDS_GUARD_MIN_EDGE, "penalty", CORQ_LOW_ODDS_GUARD_PENALTY)
...
# ----------------------------------------------------------------------
# 2026-07 Corq/Cloq selector override
# ----------------------------------------------------------------------
# Final architecture:
# - Corq page = main TOP7, selected by Corq risk-adjusted score.
# - Thinq remains an independent sanity layer.
# - Cloq = close-odds finder, winner selected by 60% Corq + 40% Thinq.

CORQ_SELECTOR_MIN_ODDS = float(os.getenv("CORQ_SELECTOR_MIN_ODDS", str(CORQ_MIN_ODDS)))
CORQ_SELECTOR_MAX_ODDS = float(os.getenv("CORQ_SELECTOR_MAX_ODDS", str(CORQ_MAX_ODDS)))
CLOQ_CLOSE_MIN_PICK_ODDS = float(os.getenv("CLOQ_CLOSE_MIN_PICK_ODDS", "1.70"))
CLOQ_CLOSE_MAX_PICK_ODDS = float(os.getenv("CLOQ_CLOSE_MAX_PICK_ODDS", str(CLOQ_MAX_PICK_ODDS)))
CLOQ_CORQ_WEIGHT = float(os.getenv("CLOQ_CORQ_WEIGHT", "0.60"))
CLOQ_THINQ_WEIGHT = float(os.getenv("CLOQ_THINQ_WEIGHT", "0.40"))
CLOQ_THINQ_MISSING_PENALTY = float(os.getenv("CLOQ_THINQ_MISSING_PENALTY", "0.04"))


def _norm_name_for_selector(value: Any) -> str:
    text = str(value or "").strip().lower()
    text = re.sub(r"[^a-z0-9]+", " ", text)
    return " ".join(text.split())


def _same_player_for_selector(a: Any, b: Any) -> bool:
    a_norm = _norm_name_for_selector(a)
    b_norm = _norm_name_for_selector(b)
...
def _probability_for_player(prediction: Dict[str, Any], probability_for_pick: Optional[float], player: Any) -> Optional[float]:
    if probability_for_pick is None:
        return None
    pick = prediction.get("pick")
    player1 = prediction.get("player1")
    player2 = prediction.get("player2")
    if _same_player_for_selector(pick, player):
        return probability_for_pick
    if _same_player_for_selector(player, player1) or _same_player_for_selector(player, player2):
        return 1.0 - probability_for_pick
    return None


def _score_pick_opp_score(prediction: Dict[str, Any]) -> Optional[Tuple[int, int]]:
    ...
    player1 = prediction.get("player1")
    player2 = prediction.get("player2")
    if _same_player_for_selector(pick, player1):
        return a, b
    if _same_player_for_selector(pick, player2):
        return b, a
    return a, b


def _sets_model_against_pick(prediction: Dict[str, Any]) -> bool:
    oriented = _score_pick_opp_score(prediction)
    if oriented is None:
        return False
    pick_sets, opponent_sets = oriented
    return pick_sets < opponent_sets


def _sets_model_supports_pick(prediction: Dict[str, Any]) -> bool:
    oriented = _score_pick_opp_score(prediction)
    if oriented is None:
        return False
    pick_sets, opponent_sets = oriented
...
def _market_lean_for_selector(prediction: Dict[str, Any]) -> str:
...
def _corq_selector_score(prediction: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    corq = get_corq_probability(prediction)
    if corq is None:
        return None
    odds = get_prediction_odds(prediction)
    if odds is None or odds < CORQ_SELECTOR_MIN_ODDS or odds > CORQ_SELECTOR_MAX_ODDS:
        return None
    ...
    # Corq is still the main model. Thinq only softens the Corq number if available.
    if thinq is not None:
        corq_q = 0.80 * corq + 0.20 * thinq
        model_gap = abs(corq - thinq)
        thinq_q = 0.50 * thinq + 0.50 * corq
        consensus_score = (corq_q + thinq_q) / 2.0
        ...
        model_gap = None
        thinq_q = None
        consensus_score = corq

    corq_q_edge = corq_q - comparison_probability if comparison_probability is not None else 0.0
    edge_bonus = min(max(corq_q_edge, 0.0) * CORQ_EDGE_BONUS_WEIGHT, CORQ_EDGE_BONUS_CAP)
    ...
    if odds < 1.45:
        penalty += 0.10
        flags.append("LOW_ODDS_LT_1_45")
    elif odds < 1.55:
        penalty += 0.08
        flags.append("LOW_ODDS_LT_1_55")
    elif odds < 1.65:
        penalty += 0.05
        flags.append("LOW_ODDS_LT_1_65")
    elif odds < 1.75:
        penalty += 0.02
        flags.append("LOW_ODDS_LT_1_75")

    if ai_match is None:
        penalty += 0.04
        flags.append("AI_MATCH_MISSING")
    elif ai_match < 85.0:
        penalty += 0.05
        flags.append("LOW_AI_MATCH_LT_85")
    elif ai_match < 90.0:
        penalty += 0.03
        flags.append("LOW_AI_MATCH_LT_90")
    elif ai_match >= 95.0:
        bonus += 0.02
        flags.append("AI_MATCH_ELITE")
    elif ai_match >= 90.0:
        bonus += 0.01
        flags.append("AI_MATCH_GOOD")

    if thinq is None:
        penalty += 0.03
        flags.append("THINQ_MISSING")

    if get_first_float(prediction, ["opponent_odds", "marq_current_opponent_odds", "marq_initial_opponent_odds"]) is None:
        odds1 = get_odds_player1(prediction)
        odds2 = get_odds_player2(prediction)
        if odds1 is None or odds2 is None:
            penalty += 0.03
            flags.append("NO_OPPONENT_ODDS")

    if _sets_model_against_pick(prediction):
        penalty += 0.08
        flags.append("SETS_MODEL_AGAINST_PICK")
    elif _sets_model_supports_pick(prediction):
        bonus += 0.015
        flags.append("SETS_MODEL_SUPPORT")

    market_lean = _market_lean_for_selector(prediction)
    if market_lean == "OPPONENT":
        penalty += 0.03
        flags.append("MARQ_LEAN_OPPONENT")
    elif market_lean == "PICK":
        bonus += 0.01
        flags.append("MARQ_LEAN_PICK")

    quality_text = " ".join(str(prediction.get(key) or "").upper() for key in ["marq_quality_signal", "marq_ai_signal", "marq_sharp_signal"])
    if "THIN" in quality_text or "NO SHARP DATA" in quality_text:
        penalty += 0.01
        flags.append("THIN_MARKET_DATA")

    elo_found_1 = prediction.get("elo_found_player1")
    elo_found_2 = prediction.get("elo_found_player2")
    if elo_found_1 is False or elo_found_2 is False:
        penalty += 0.05
    ...
    final_score = corq_q + bonus - penalty
    return {
        "corq": corq,
        "thinq": thinq,
        "corq_q": corq_q,
...
def derive_corq_predictions(all_predictions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for prediction in all_predictions:
        selector = _corq_selector_score(prediction)
        if not selector:
            continue
        updated = clone_prediction(prediction)
        updated["probability"] = round(selector["corq_q"], 4)
        updated["corq_raw_probability"] = round(selector["corq"], 4)
        updated["thinq_raw_probability"] = round(selector["thinq"], 4) if selector["thinq"] is not None else None
        updated["corq_q_probability"] = round(selector["corq_q"], 4)
        updated["thinq_q_probability"] = round(selector["thinq_q"], 4) if selector["thinq_q"] is not None else None
        updated["cloq_ai_probability"] = round(selector["consensus_score"], 4)
        updated["model_gap"] = round(selector["model_gap"], 4) if selector["model_gap"] is not None else None
        updated["consensus_score"] = round(selector["consensus_score"], 4)
        updated["market_break_even"] = round(selector["market_break_even"], 4)
        updated["market_probability"] = round(selector["market_probability"], 4)
        updated["corq_q_edge"] = round(selector["corq_q_edge"], 4)
        updated["corq_adjusted_score"] = round(selector["final_score"], 4)
        updated["corq_top_score"] = round(selector["final_score"], 4)
        updated["corq_edge_bonus"] = round(selector["bonus"], 4)
        updated["corq_bonus"] = round(selector["bonus"], 4)
        updated["corq_risk_penalty"] = round(selector["penalty"], 4)
        updated["corq_risk_flags"] = selector["flags"]
        updated["consensus_status"] = consensus_status(selector["model_gap"])
        updated["top_mode"] = "CORQ_TOP7_RISK_ADJUSTED"
        updated["top_reason"] = "Corq TOP7: main Corq model ranked by CorqQ with risk penalties; Thinq is a soft sanity signal, not the TOP7 selector."
        eligible.append(updated)

    eligible = dedupe_predictions(eligible)
    eligible.sort(
        key=lambda item: (
            safe_float(item.get("corq_top_score")) or 0.0,
            safe_float(item.get("corq_q_probability")) or 0.0,
            safe_float(item.get("odds")) or 0.0,
        ),
        reverse=True,
...
def _cloq_side_context(prediction: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    corq_pick = get_corq_probability(prediction)
    if corq_pick is None:
        return None
    thinq_pick = get_thinq_probability(prediction)
    player1 = prediction.get("player1")
    player2 = prediction.get("player2")
    pick = prediction.get("pick")
    ...
    corq_p1 = _probability_for_player(prediction, corq_pick, player1)
    if corq_p1 is None:
        return None
    if thinq_pick is not None:
        thinq_p1 = _probability_for_player(prediction, thinq_pick, player1)
        if thinq_p1 is None:
            thinq_p1 = corq_p1
        cloq_p1 = CLOQ_CORQ_WEIGHT * corq_p1 + CLOQ_THINQ_WEIGHT * thinq_p1
        thinq_available = True
    else:
        thinq_p1 = None
        cloq_p1 = max(0.0, min(1.0, corq_p1 - CLOQ_THINQ_MISSING_PENALTY))
        thinq_available = False

    cloq_p2 = 1.0 - cloq_p1
    if cloq_p1 >= cloq_p2:
        selected_player = player1
        selected_opponent = player2
        selected_probability = cloq_p1
        selected_odds = odds1
    else:
        selected_player = player2
        selected_opponent = player1
        selected_probability = cloq_p2
        selected_odds = odds2

    if selected_odds is None or selected_odds < CLOQ_CLOSE_MIN_PICK_ODDS or selected_odds > CLOQ_CLOSE_MAX_PICK_ODDS:
        return None

    close_bonus = 0.03 if market_gap <= 0.05 else 0.02 if market_gap <= CLOQ_STRONG_MARKET_GAP else 0.01
    odds_bonus = 0.02 if selected_odds >= 1.90 else 0.01 if selected_odds >= 1.75 else 0.0
    ai_match = get_ai_match(prediction)
    ai_bonus = 0.02 if ai_match is not None and ai_match >= 95 else 0.01 if ai_match is not None and ai_match >= 90 else 0.0
    risk_penalty = 0.03 if not thinq_available else 0.0
    cloq_score = selected_probability + close_bonus + odds_bonus + ai_bonus - risk_penalty
    ...
    return {
        ...
        "thinq_p1": thinq_p1,
        "cloq_p1": cloq_p1,
        "cloq_score": cloq_score,
        "thinq_available": thinq_available,
        "ai_match": ai_match,
    }
...
def derive_cloq_predictions(all_predictions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    eligible: List[Dict[str, Any]] = []
    for prediction in all_predictions:
        ctx = _cloq_side_context(prediction)
        if not ctx:
            continue
        updated = clone_prediction(prediction)
        old_pick = updated.get("pick")
        if not _same_player_for_selector(old_pick, ctx["selected_player"]):
            updated["cloq_original_corq_pick"] = old_pick
        updated["pick"] = ctx["selected_player"]
        updated["opponent"] = ctx["selected_opponent"]
        updated["odds"] = ctx["selected_odds"]
        updated["pick_odds"] = ctx["selected_odds"]
        updated["opponent_odds"] = ctx["odds2"] if _same_player_for_selector(ctx["selected_player"], updated.get("player1")) else ctx["odds1"]
        updated["probability"] = round(ctx["selected_probability"], 4)
        updated["cloq_probability"] = round(ctx["selected_probability"], 4)
        updated["cloq_ai_probability"] = round(ctx["selected_probability"], 4)
        updated["cloq_score"] = round(ctx["cloq_score"], 4)
        updated["market_fair_player1"] = round(ctx["fair1"], 4) if ctx["fair1"] is not None else None
        updated["market_fair_player2"] = round(ctx["fair2"], 4) if ctx["fair2"] is not None else None
        updated["market_gap"] = round(ctx["market_gap"], 4)
        updated["cloq_market_quality"] = calculate_cloq_market_quality(ctx["market_gap"])
        updated["cloq_corq_weight"] = CLOQ_CORQ_WEIGHT
        updated["cloq_thinq_weight"] = CLOQ_THINQ_WEIGHT if ctx["thinq_available"] else 0.0
        updated["cloq_thinq_available"] = ctx["thinq_available"]
        updated["cloq_model_gap_bucket"] = cloq_model_gap_bucket(safe_float(updated.get("model_gap")))
        updated["cloq_tier"] = "CLOQ_CLOSE_STRONG" if ctx["market_gap"] <= CLOQ_STRONG_MARKET_GAP else "CLOQ_CLOSE"
        updated["top_mode"] = "CLOQ_CLOSE_ODDS_60_40"
        updated["top_reason"] = "Cloq: close-odds finder; winner selected by 60% Corq + 40% Thinq when available."
        eligible.append(updated)
    ...
    eligible.sort(
        key=lambda item: (
            ...
            -(safe_float(item.get("market_gap")) or 1.0),
            safe_float(item.get("odds")) or 0.0,
        ),
        reverse=True,
    )
