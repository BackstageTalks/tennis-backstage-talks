...
from tennisapi_cache import get_daily_odds_items, betting_day_datetime, normalize_event_market_odds_payload
...
def get_tennisapi_odds(match_id: int) -> Optional[Dict[str, Any]]:
    """Try TennisApi PRO event-level odds endpoints in priority order."""
    try:
    ...
    try:
        ...
        if normalized:
            ...
            normalized["source"] = "TennisApiMatchWinningOdds"
            normalized["odds_source"] = "TennisApiMatchWinningOdds"
            return normalized
    ...
    try:
        payload = client.get_match_betting_odds(match_id)
        normalized = normalize_event_market_odds_payload(payload)
        if normalized:
            normalized["match_id"] = match_id
            normalized["source"] = "TennisApiMatchBettingOdds"
            normalized["odds_source"] = "TennisApiMatchBettingOdds"
            return normalized
    ...
    try:
        payload = client.get_all_odds_for_event(match_id)
        normalized = normalize_event_market_odds_payload(payload)
        if normalized:
            normalized["match_id"] = match_id
            normalized["source"] = "TennisApiAllOddsForEvent"
            normalized["odds_source"] = "TennisApiAllOddsForEvent"
            return normalized
    ...
    try:
        payload = client.get_match_featured_odds(match_id)
        normalized = normalize_event_market_odds_payload(payload)
        if normalized:
            normalized["match_id"] = match_id
            normalized["source"] = "TennisApiMatchFeaturedOdds"
            normalized["odds_source"] = "TennisApiMatchFeaturedOdds"
            return normalized
        attempts.append("getMatchFeaturedOdds:no_data")
    except Exception as exc:
        attempts.append(f"getMatchFeaturedOdds:error:{exc}")

    logger.info("TennisApi event odds failed. match_id=%s attempts=%s", match_id, " | ".join(attempts))
    return None
...
def _to_legacy_odds_item(odds: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "source": odds.get("odds_source") or odds.get("source", "TennisApi"),
        "odds_source": odds.get("odds_source") or odds.get("source", "TennisApi"),
