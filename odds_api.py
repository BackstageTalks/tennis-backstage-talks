...
def get_tennisapi_odds(match_id: int) -> Optional[Dict[str, Any]]:
    """
    TennisApi PRO event-level odds fallback chain.

    This function is used when a match was not found in the combined daily odds
    list. It must therefore try all PRO odds endpoints before returning None.
    """
    try:
