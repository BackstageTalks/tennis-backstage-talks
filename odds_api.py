...
def fetch_odds(*args: Any, **kwargs: Any) -> List[Dict[str, Any]]:
    """Fetch the broad daily odds list.

    Important: fallback APIs are also tried later inside find_match_odds()
    per event_id, because they are event-level endpoints and may have request
    limits. This function intentionally preserves the old working daily list.
    """
...
def find_match_odds(*args: Any, **kwargs: Any) -> Optional[Dict[str, Any]]:
    """Find odds for a match.

    Priority:
    1) ID match in daily odds list.
    2) Event-level API chain using the same event_id:
       TennisApi PRO -> Tennis API ATP/WTA/ITF -> All Sports API.
    3) Normalized name matching, reversed order and fuzzy fallback.
    4) Write public/no_odds_report.json with a concrete reason.
    """
    ...
    # 1) exact event id / match id from daily odds list
    if match_id:
        for item in odds_list:
            ...
            if item_id is not None and str(item_id) == str(match_id):
                ...
                out.setdefault("no_odds_reason", None)
                return out

        # 2) active event-level fallback chain
        ...
        if direct:
            ...
            item["no_odds_reason"] = None
            return item

    # 3) name matching in daily odds list
    legacy = _find_in_legacy_odds_list(odds_list, player1, player2)
    if legacy:
        legacy.setdefault("no_odds_reason", None)
        return legacy

    _record_no_odds(match_id, player1, player2, tournament, start_time, "no_daily_match_and_event_fallback_failed")
    return None


# ----------------------------------------------------------------------
# Event-level fallback chain
# ----------------------------------------------------------------------
    ...
    for provider_name, func in (
        ...
        try:
            ...
            if odds:
                ...
                odds.setdefault("fallback_tried_sources", tried)
                return odds
        except Exception as exc:
...
def get_tennisapi_odds(match_id: int) -> Optional[Dict[str, Any]]:
    try:
        ...
        logger.info("TennisApi event odds failed match_id=%s error=%s", match_id, exc)
    return None


def _rapid_key(env_name: str) -> str:
    return (
...
def get_tennis_api_atp_wta_itf_odds(event_id: int) -> Optional[Dict[str, Any]]:
    key = _rapid_key("TENNIS_API_ATP_WTA_ITF")
    if not key:
        return None
    ...
    paths = [
        ...
        f"/tennis/v2/extend/api/odds/recent/{event_id}",
    ]
    for path in paths:
...
def _rapidapi_get_json(host: str, path: str, api_key: str, timeout: int = 20) -> Dict[str, Any]:
    try:
        ...
        if res.status >= 400:
            logger.debug("RapidAPI HTTP %s host=%s path=%s body=%s", res.status, host, path, raw[:500])
            return {}
        if not raw:
            return {}
        data = json.loads(raw)
        ...
        logger.debug("RapidAPI request failed host=%s path=%s error=%s", host, path, exc)
        return {}
...
def _normalize_generic_event_odds_payload(payload: Any) -> Optional[Dict[str, Any]]:
    ...
    for obj in candidates:
        ...
        if p1 and p2 and p1 > 1.0 and p2 > 1.0:
            return _odds_dict(p1, p2, obj.get("bookmaker") or obj.get("provider") or obj.get("source"))
...
def _extract_decimal_from_dict(obj: Dict[str, Any]) -> Optional[float]:
    for key in ("decimalValue", "decimal", "price", "odds", "value", "currentOdd", "od", "od1", "od2"):
        dec = _to_float_or_none(obj.get(key))
        if dec and dec > 1.0:
            return round(dec, 4)
...
def _to_legacy_odds_item(odds: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "fallback_tried_sources": odds.get("fallback_tried_sources"),
        "no_odds_reason": odds.get("no_odds_reason"),
    }
