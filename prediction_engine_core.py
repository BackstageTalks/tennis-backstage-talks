...
def normalize_match(match):
    """
    Keep all fields the model needs, including TennisApi event identifiers.
    Earlier versions dropped match_id/event_id, which made direct odds lookup impossible.
    """
    match_id = (
        match.get("match_id")
...
def build_prediction_record(
    ...
    return {
        ...
        "expected_sets": match_info.get("expected_sets"),
        "sets_probability": match_info.get("sets_probability"),
        "sets_probability_label": match_info.get("sets_probability_label"),
        "set_win_probability": match_info.get("set_win_probability"),
        "most_likely_score": match_info.get("most_likely_score"),
        "most_likely_score_probability": match_info.get("most_likely_score_probability"),
        "score_probabilities": match_info.get("score_probabilities"),
        "expected_games": match_info.get("expected_games"),
        "games_pick": match_info.get("games_pick"),
        "games_line": match_info.get("games_line"),
        "bet_tag": bet_tag,
...
def build_all_predictions():
    ...
    for match in matches:
        ...
        # Current best pattern: pass the whole match and odds list.
        # This allows direct match_id/event_id lookup when available, plus name matching fallback.
