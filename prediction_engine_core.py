...
def build_thinq_h2h_fields(h2h_info):
    h2h_info = h2h_info or {}
    total = safe_float(h2h_info.get("h2h_total_matches")) or 0.0
    signal = str(h2h_info.get("h2h_signal") or "NO_DATA").upper()
    source = h2h_info.get("h2h_source") or "none"
    status = "AVAILABLE" if total > 0 and signal != "NO_DATA" else "NO_DATA"

    edge = safe_float(h2h_info.get("h2h_adjustment"))
    if edge is None:
        adjustment_pct = safe_float(h2h_info.get("h2h_adjustment_pct"))
        edge = adjustment_pct / 100.0 if adjustment_pct is not None else None
    if status != "AVAILABLE":
        edge = None
    ...
    if status != "AVAILABLE":
        ...
        flags.append("H2H_SMALL_SAMPLE")

    return {
        "thinq_h2h_status": status,
        "thinq_h2h_edge": round(edge, 4) if edge is not None else None,
        "thinq_h2h_signal": signal,
        "thinq_h2h_source": source,
        "thinq_h2h_total_matches": int(total),
        "thinq_h2h_pick_wins": h2h_info.get("h2h_pick_wins"),
        "thinq_h2h_opponent_wins": h2h_info.get("h2h_opponent_wins"),
        "thinq_h2h_confidence": round(confidence, 4),
        "thinq_h2h_reason": h2h_info.get("h2h_reason"),
        "thinq_h2h_flags": flags,
    }
...
def build_prediction_record(
    ...
    return {
        ...
        "h2h_source": h2h_info.get("h2h_source"),
        "h2h_endpoint_path": h2h_info.get("h2h_endpoint_path"),
        "h2h_api_total_matches": h2h_info.get("h2h_api_total_matches"),
        "h2h_sackmann_total_matches": h2h_info.get("h2h_sackmann_total_matches"),
    ...
    prediction = attach_thinq_to_prediction(
    ...
    # H2H is owned by THINQ. Keep legacy h2h_* fields for audit, but expose
    # explicit THINQ H2H fields so CORQ/UI do not treat NO_DATA as 0.0%.
    thinq_h2h_fields = build_thinq_h2h_fields(h2h_info)
    prediction.update(thinq_h2h_fields)

    flags = prediction.get("thinq_flags")
    if not isinstance(flags, list):
        flags = []
    for flag in thinq_h2h_fields.get("thinq_h2h_flags", []):
        if flag not in flags:
            flags.append(flag)
    prediction["thinq_flags"] = flags

    if thinq_h2h_fields.get("thinq_h2h_status") == "AVAILABLE":
        prediction["thinq_h2h_edge"] = thinq_h2h_fields.get("thinq_h2h_edge")
    else:
        prediction["thinq_h2h_edge"] = None
