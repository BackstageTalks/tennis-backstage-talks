...
from thinq.loaders.surface_loader import resolve_tournament_surface
...
def normalize_match(match):
    ...
    tournament_obj = match.get("tournament") if isinstance(match.get("tournament"), dict) else {}
    unique_tournament_obj = (
        match.get("uniqueTournament")
        if isinstance(match.get("uniqueTournament"), dict)
        else tournament_obj.get("uniqueTournament")
        if isinstance(tournament_obj.get("uniqueTournament"), dict)
        else {}
    )
    category_obj = match.get("category") if isinstance(match.get("category"), dict) else {}
    ...
    tournament_id = (
        ...
        or tournament_obj.get("id")
    )
    unique_tournament_id = (
        match.get("unique_tournament_id")
        or match.get("uniqueTournamentId")
        or match.get("unique_tournamentId")
        or unique_tournament_obj.get("id")
    )

    tournament_name = match.get("tournament")
    if isinstance(tournament_name, dict):
        tournament_name = (
            tournament_name.get("name")
            or tournament_name.get("slug")
            or tournament_name.get("competitionName")
        )

    category_name = match.get("category")
    if isinstance(category_name, dict):
        category_name = category_name.get("name") or category_name.get("slug")
    ...
    return {
        ...
        "surface_source": match.get("surface_source") or match.get("surfaceSource"),
        "surface_raw": match.get("surface_raw") or match.get("surfaceRaw") or match.get("surfaceType"),
        "surfaceType": match.get("surfaceType"),

        "tournament_id": tournament_id,
        "tournamentId": tournament_id,
        "unique_tournament_id": unique_tournament_id,
        "uniqueTournamentId": unique_tournament_id,
        "competition_id": match.get("competition_id") or match.get("competitionId"),
        "competitionId": match.get("competitionId") or match.get("competition_id"),
        "category_id": match.get("category_id") or match.get("categoryId") or category_obj.get("id"),
        "categoryId": match.get("categoryId") or match.get("category_id") or category_obj.get("id"),

        "tournament": tournament_name,
        "competitionName": match.get("competitionName") or tournament_name,
...
def infer_surface_info(surface_map, player1, player2, match):
    """Resolve surface through THINQ surface resolver.

    Priority:
    1. TennisApi PRO tournament metadata via thinq/loaders/surface_loader.py
    2. Explicit match surface, if present and resolver cannot resolve
    3. stats_engine surface_map, if present
    4. Unknown, never default Hard
    """
    tournament_id = (
        match.get("tournament_id")
        or match.get("tournamentId")
    ...
    tournament_name = match.get("tournament") or match.get("competitionName") or match.get("event_name")

    try:
        info = resolve_tournament_surface(
            tournament_id=tournament_id,
            tournament_name=tournament_name,
            match=match,
        )
    except Exception as exc:
        print("SURFACE RESOLVER ERROR:", player1, "vs", player2, str(exc))
        info = {
            "surface": "Unknown",
            "surface_raw": None,
            "surface_environment": None,
            "surface_model_bucket": "Unknown",
            "surface_source": "surface_resolver_error",
            "surface_confidence": "LOW",
            "thinq_selected_elo_type": "overall_elo",
            "surface_flags": ["SURFACE_RESOLVER_ERROR"],
        }

    if info.get("surface") and info.get("surface") != "Unknown":
        return info

    explicit_surface = match.get("surface")
    explicit_source = str(match.get("surface_source") or match.get("surfaceSource") or "").lower()
    trusted_match_surface = explicit_source in {
        "tennisapi_gettournamentinfometadata",
        "tennisapi_tournament_metadata",
        "api_tournament_metadata",
        "manual_verified",
    }
    if explicit_surface and trusted_match_surface:
        raw = str(explicit_surface).strip()
        lowered = raw.lower()
        if "clay" in lowered:
            model_surface = "Clay"
            elo_type = "clay_elo"
        elif "grass" in lowered:
            model_surface = "Grass"
            elo_type = "grass_elo"
        elif "carpet" in lowered:
            model_surface = "Carpet"
            elo_type = "hard_elo"
        elif "hard" in lowered:
            model_surface = "Hard"
            elo_type = "hard_elo"
        else:
            model_surface = raw or "Unknown"
            elo_type = "overall_elo"
        flags = ["TRUSTED_MATCH_SURFACE_FALLBACK"]
        if model_surface == "Carpet":
            flags.append("CARPET_AS_HARD_FALLBACK")
        return {
            "surface": model_surface,
            "surface_raw": raw,
            "surface_environment": "Indoor" if "indoor" in lowered else ("Outdoor" if "outdoor" in lowered else None),
            "surface_model_bucket": "Hard" if model_surface == "Carpet" else model_surface,
            "surface_source": "trusted_match_surface_fallback",
            "surface_confidence": "MEDIUM",
            "thinq_selected_elo_type": elo_type,
            "surface_flags": flags,
        }

    mapped = surface_map.get(f"{player1}::{player2}") if isinstance(surface_map, dict) else None
    if mapped:
        return {
            "surface": mapped,
            "surface_raw": mapped,
            "surface_environment": None,
            "surface_model_bucket": mapped,
            "surface_source": "stats_engine_surface_map",
            "surface_confidence": "MEDIUM",
            "thinq_selected_elo_type": {"Clay": "clay_elo", "Grass": "grass_elo", "Hard": "hard_elo"}.get(mapped, "overall_elo"),
            "surface_flags": [],
        }
    ...
    return {
        ...
        "surface_confidence": "LOW",
        "thinq_selected_elo_type": "overall_elo",
        "surface_flags": ["UNKNOWN_SURFACE"],
    }


def infer_surface(surface_map, player1, player2, match):
    return infer_surface_info(surface_map, player1, player2, match).get("surface") or "Unknown"
...
def build_prediction_record(
    ...
    return {
        ...
        "surface": surface,
        "surface_raw": surface_info.get("surface_raw"),
        "surface_environment": surface_info.get("surface_environment"),
        "surface_model_bucket": surface_info.get("surface_model_bucket") or surface,
        "surface_source": surface_info.get("surface_source"),
        "surface_confidence": surface_info.get("surface_confidence"),
        "surface_flags": surface_info.get("surface_flags"),
        "thinq_selected_elo_type": surface_info.get("thinq_selected_elo_type"),
...
def build_all_predictions():
    ...
    for match in matches:
        surface_info = infer_surface_info(
            surface_map,
            match["player1"],
            match["player2"],
            match,
        )
        surface = surface_info.get("surface") or "Unknown"
