...
def normalize_surface_text(value):
    ...
    if not text:
        return ""
    if "clay" in text or "antuka" in text:
        return "Clay"
    if "grass" in text:
        return "Grass"
    if "carpet" in text:
...
def normalize_tournament_text(value):
...
TOURNAMENT_SURFACE_OVERRIDES = {
    # Current July clay swing / verified problem tournaments
    "estoril": "Clay",
    "millennium estoril open": "Clay",
    "palermo": "Clay",
    "palermo ladies open": "Clay",
    "wta 125k palermo": "Clay",
    "37 palermo ladies open": "Clay",
    "kitzbuhel": "Clay",
    "kitzbuehel": "Clay",
    "generali open": "Clay",
    "hamburg": "Clay",
    ...
    # Common hard-court summer events, to avoid unknown fallback guessing later
    "los cabos": "Hard",
    "washington": "Hard",
    "atlanta": "Hard",
    "montreal": "Hard",
...
def tournament_surface_override(match):
    parts = [
    ...
    blob = normalize_tournament_text(" ".join(str(part or "") for part in parts))
    if not blob:
        return None
    for key, surface in TOURNAMENT_SURFACE_OVERRIDES.items():
        key_norm = normalize_tournament_text(key)
...
def infer_surface(surface_map, player1, player2, match):
    ...
    Important: many event APIs omit surface and older pipeline code fell back to Hard.
    During clay weeks that silently poisoned ELO/Form/THINQ calculations. Tournament
    overrides are therefore applied before accepting a generic Hard value from event data.
    """
    override_surface = tournament_surface_override(match)
    if override_surface:
        return override_surface

    explicit_surface = normalize_surface_text(match.get("surface"))
    if explicit_surface:
        return explicit_surface

    mapped_surface = normalize_surface_text(surface_map.get(f"{player1}::{player2}"))
    if mapped_surface:
